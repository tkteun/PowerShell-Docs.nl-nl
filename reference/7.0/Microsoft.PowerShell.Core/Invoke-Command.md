...
### <span data-ttu-id="c5fef-109">Sessie</span><span class="sxs-lookup"><span data-stu-id="c5fef-109">Session</span></span>
...
### <span data-ttu-id="c5fef-119">ContainerId</span><span class="sxs-lookup"><span data-stu-id="c5fef-119">ContainerId</span></span>
...
<span data-ttu-id="c5fef-151">Als antwoord Power shell vraagt het wacht woord en een verificatie methode voor het gebruiker01-account.</span><span class="sxs-lookup"><span data-stu-id="c5fef-151">In response, PowerShell requests the password and an authentication method for the User01 account.</span></span>
...
<span data-ttu-id="c5fef-174">In dit voor beeld ziet u hoe u een opdracht maakt die is opgeslagen als een script blok in een lokale variabele.</span><span class="sxs-lookup"><span data-stu-id="c5fef-174">This example shows how to create a command that is stored as a script block in a local variable.</span></span>
...
### <span data-ttu-id="c5fef-184">Voor beeld 7: de versie van het hostprogramma op meerdere computers ophalen</span><span class="sxs-lookup"><span data-stu-id="c5fef-184">Example 7: Get the version of the host program on multiple computers</span></span>
...
### <span data-ttu-id="c5fef-207">Voor beeld 9: lokale variabelen insluiten in een opdracht die wordt uitgevoerd op een externe computer</span><span class="sxs-lookup"><span data-stu-id="c5fef-207">Example 9: Include local variables in a command run on a remote computer</span></span>
...
<span data-ttu-id="c5fef-236">`Invoke-Command` Hiermee wordt een **script Block** -para meter gebruikt en worden `Get-ChildItem` de-en- `$args[0]` `$args[1]` matrix waarden opgegeven.</span><span class="sxs-lookup"><span data-stu-id="c5fef-236">`Invoke-Command` uses a **ScriptBlock** parameter and `Get-ChildItem` specifies the `$args[0]` and `$args[1]` array values.</span></span> <span data-ttu-id="c5fef-237">De **argument List** geeft de `$args` matrix waarden door aan de `Get-ChildItem` parameter posities voor **Path** en **filter**.</span><span class="sxs-lookup"><span data-stu-id="c5fef-237">The **ArgumentList** passes the `$args` array values to the `Get-ChildItem` parameter positions for **Path** and **Filter**.</span></span>
...
### <span data-ttu-id="c5fef-303">Voor beeld 20: een opdracht uitvoeren op een externe computer met SSH en een gebruikers verificatie sleutel opgeven</span><span class="sxs-lookup"><span data-stu-id="c5fef-303">Example 20: Run a command on a remote computer using SSH and specify a user authentication key</span></span>
...
<span data-ttu-id="c5fef-312">Hiermee kan de verbinding worden omgeleid naar een alternatieve URI (Uniform Resource Identifier).</span><span class="sxs-lookup"><span data-stu-id="c5fef-312">Allows redirection of this connection to an alternate Uniform Resource Identifier (URI).</span></span>
...
<span data-ttu-id="c5fef-325">De WinRM-service gebruikt de naam van de toepassing om een listener te selecteren om de verbindings aanvraag te onderhouden.</span><span class="sxs-lookup"><span data-stu-id="c5fef-325">The WinRM service uses the application name to select a listener to service the connection request.</span></span>
...
### <span data-ttu-id="c5fef-395">-ConnectionUri</span><span class="sxs-lookup"><span data-stu-id="c5fef-395">-ConnectionUri</span></span>
...
<span data-ttu-id="c5fef-399">De standaard waarde is als volgt:</span><span class="sxs-lookup"><span data-stu-id="c5fef-399">The default value is as follows:</span></span>

`http://localhost:5985/WSMAN`

<span data-ttu-id="c5fef-400">Als u geen verbindings-URI opgeeft, kunt u de para meters **UseSSL** en **Port** gebruiken om de verbindings-URI-waarden op te geven.</span><span class="sxs-lookup"><span data-stu-id="c5fef-400">If you don't specify a connection URI, you can use the **UseSSL** and **Port** parameters to specify the connection URI values.</span></span>

<span data-ttu-id="c5fef-401">Geldige waarden voor het **transport** segment van de URI zijn http en HTTPS.</span><span class="sxs-lookup"><span data-stu-id="c5fef-401">Valid values for the **Transport** segment of the URI are HTTP and HTTPS.</span></span> <span data-ttu-id="c5fef-402">Als u een verbindings-URI met een transport segment opgeeft, maar geen poort opgeeft, wordt de sessie gemaakt met de standaard poorten: 80 voor HTTP en 443 voor HTTPS.</span><span class="sxs-lookup"><span data-stu-id="c5fef-402">If you specify a connection URI with a Transport segment, but don't specify a port, the session is created with the standards ports: 80 for HTTP and 443 for HTTPS.</span></span> <span data-ttu-id="c5fef-403">Als u de standaard poorten voor externe communicatie van Power shell wilt gebruiken, geeft u poort 5985 voor HTTP of 5986 op voor HTTPS.</span><span class="sxs-lookup"><span data-stu-id="c5fef-403">To use the default ports for PowerShell remoting, specify port 5985 for HTTP or 5986 for HTTPS.</span></span>

<span data-ttu-id="c5fef-404">Als de doel computer de verbinding omleidt naar een andere URI, wordt de omleiding door Power shell voor komen, tenzij u de para meter **AllowRedirection** in de opdracht gebruikt.</span><span class="sxs-lookup"><span data-stu-id="c5fef-404">If the destination computer redirects the connection to a different URI, PowerShell prevents the redirection unless you use the **AllowRedirection** parameter in the command.</span></span>
...
### <span data-ttu-id="c5fef-405">-ContainerId</span><span class="sxs-lookup"><span data-stu-id="c5fef-405">-ContainerId</span></span>

<span data-ttu-id="c5fef-406">Hiermee geeft u een matrix met container-Id's op.</span><span class="sxs-lookup"><span data-stu-id="c5fef-406">Specifies an array of container IDs.</span></span>
...
### <span data-ttu-id="c5fef-407">-Credential</span><span class="sxs-lookup"><span data-stu-id="c5fef-407">-Credential</span></span>

<span data-ttu-id="c5fef-408">Hiermee geeft u een gebruikers account op dat is gemachtigd om deze actie uit te voeren.</span><span class="sxs-lookup"><span data-stu-id="c5fef-408">Specifies a user account that has permission to perform this action.</span></span> <span data-ttu-id="c5fef-409">Standaard is dit de huidige gebruiker.</span><span class="sxs-lookup"><span data-stu-id="c5fef-409">The default is the current user.</span></span>

<span data-ttu-id="c5fef-410">Typ een gebruikers naam, zoals **gebruiker01** of **Domain01\User01** , of voer een **PSCredential** -object in dat door de cmdlet wordt gegenereerd `Get-Credential` .</span><span class="sxs-lookup"><span data-stu-id="c5fef-410">Type a user name, such as **User01** or **Domain01\User01** , or enter a **PSCredential** object generated by the `Get-Credential` cmdlet.</span></span> <span data-ttu-id="c5fef-411">Als u een gebruikers naam typt, wordt u gevraagd het wacht woord in te voeren.</span><span class="sxs-lookup"><span data-stu-id="c5fef-411">If you type a user name, you're prompted to enter the password.</span></span>

<span data-ttu-id="c5fef-412">Referenties worden opgeslagen in een [PSCredential](/dotnet/api/system.management.automation.pscredential) -object en het wacht woord wordt opgeslagen als [SecureString](/dotnet/api/system.security.securestring).</span><span class="sxs-lookup"><span data-stu-id="c5fef-412">Credentials are stored in a [PSCredential](/dotnet/api/system.management.automation.pscredential) object and the password is stored as a [SecureString](/dotnet/api/system.security.securestring).</span></span>

> [!NOTE]
> <span data-ttu-id="c5fef-413">Zie [Hoe veilig is securestring?](/dotnet/api/system.security.securestring#how-secure-is-securestring)voor meer informatie over **securestring** Data Protection.</span><span class="sxs-lookup"><span data-stu-id="c5fef-413">For more information about **SecureString** data protection, see [How secure is SecureString?](/dotnet/api/system.security.securestring#how-secure-is-securestring).</span></span>
...
### <span data-ttu-id="c5fef-414">-EnableNetworkAccess</span><span class="sxs-lookup"><span data-stu-id="c5fef-414">-EnableNetworkAccess</span></span>

<span data-ttu-id="c5fef-415">Geeft aan dat met deze cmdlet een interactief beveiligings token wordt toegevoegd aan loop Back-sessies.</span><span class="sxs-lookup"><span data-stu-id="c5fef-415">Indicates that this cmdlet adds an interactive security token to loopback sessions.</span></span> <span data-ttu-id="c5fef-416">Met het interactieve token kunt u opdrachten uitvoeren in de loop back-sessie die gegevens van andere computers ophalen.</span><span class="sxs-lookup"><span data-stu-id="c5fef-416">The interactive token lets you run commands in the loopback session that get data from other computers.</span></span> <span data-ttu-id="c5fef-417">U kunt bijvoorbeeld een opdracht uitvoeren in de sessie waarmee XML-bestanden van een externe computer naar de lokale computer worden gekopieerd.</span><span class="sxs-lookup"><span data-stu-id="c5fef-417">For example, you can run a command in the session that copies XML files from a remote computer to the local computer.</span></span>

<span data-ttu-id="c5fef-418">Een loop back-sessie is een **PSSession** die afkomstig is van en eindigt op dezelfde computer.</span><span class="sxs-lookup"><span data-stu-id="c5fef-418">A loopback session is a **PSSession** that originates and ends on the same computer.</span></span> <span data-ttu-id="c5fef-419">Als u een loop back-sessie wilt maken, laat u de para meter **ComputerName** weg of stelt u de waarde in op punt ( `.` ), localhost of de naam van de lokale computer.</span><span class="sxs-lookup"><span data-stu-id="c5fef-419">To create a loopback session, omit the **ComputerName** parameter or set its value to dot (`.`), localhost, or the name of the local computer.</span></span>

<span data-ttu-id="c5fef-420">Loop Back-sessies worden standaard gemaakt met behulp van een netwerk token, wat mogelijk niet voldoende machtigingen biedt om te verifiëren bij externe computers.</span><span class="sxs-lookup"><span data-stu-id="c5fef-420">By default, loopback sessions are created using a network token, which might not provide sufficient permission to authenticate to remote computers.</span></span>

<span data-ttu-id="c5fef-421">De para meter **EnableNetworkAccess** is alleen effectief in loop Back-sessies.</span><span class="sxs-lookup"><span data-stu-id="c5fef-421">The **EnableNetworkAccess** parameter is effective only in loopback sessions.</span></span> <span data-ttu-id="c5fef-422">Als u **EnableNetworkAccess** gebruikt wanneer u een sessie op een externe computer maakt, mislukt de opdracht, maar wordt de para meter genegeerd.</span><span class="sxs-lookup"><span data-stu-id="c5fef-422">If you use **EnableNetworkAccess** when you create a session on a remote computer, the command succeeds, but the parameter is ignored.</span></span>

<span data-ttu-id="c5fef-423">U kunt externe toegang toestaan in een loop back-sessie met behulp van de **CredSSP** -waarde van de **verificatie** parameter, waarmee de sessie referenties worden gedelegeerd aan andere computers.</span><span class="sxs-lookup"><span data-stu-id="c5fef-423">You can allow remote access in a loopback session using the **CredSSP** value of the **Authentication** parameter, which delegates the session credentials to other computers.</span></span>

<span data-ttu-id="c5fef-424">Om de computer te beschermen tegen kwaadwillende toegang, kunnen niet-verbonden loop Back-sessies met interactieve tokens, die zijn gemaakt met behulp van **EnableNetworkAccess** , alleen opnieuw verbinding maken vanaf de computer waarop de sessie is gemaakt.</span><span class="sxs-lookup"><span data-stu-id="c5fef-424">To protect the computer from malicious access, disconnected loopback sessions that have interactive tokens, which are those created using **EnableNetworkAccess** , can be reconnected only from the computer on which the session was created.</span></span> <span data-ttu-id="c5fef-425">Verbroken sessies die gebruikmaken van CredSSP-verificatie, kunnen opnieuw worden aangesloten op andere computers.</span><span class="sxs-lookup"><span data-stu-id="c5fef-425">Disconnected sessions that use CredSSP authentication can be reconnected from other computers.</span></span> <span data-ttu-id="c5fef-426">Voor meer informatie raadpleegt u `Disconnect-PSSession`.</span><span class="sxs-lookup"><span data-stu-id="c5fef-426">For more information, see `Disconnect-PSSession`.</span></span>

<span data-ttu-id="c5fef-427">Deze para meter is geïntroduceerd in Power Shell 3,0.</span><span class="sxs-lookup"><span data-stu-id="c5fef-427">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="c5fef-428">-FilePath</span><span class="sxs-lookup"><span data-stu-id="c5fef-428">-FilePath</span></span>

<span data-ttu-id="c5fef-429">Hiermee geeft u een lokaal script op dat met deze cmdlet wordt uitgevoerd op een of meer externe computers.</span><span class="sxs-lookup"><span data-stu-id="c5fef-429">Specifies a local script that this cmdlet runs on one or more remote computers.</span></span> <span data-ttu-id="c5fef-430">Voer het pad en de bestands naam van het script in of pipet een scriptpad naar `Invoke-Command` .</span><span class="sxs-lookup"><span data-stu-id="c5fef-430">Enter the path and filename of the script, or pipe a script path to `Invoke-Command`.</span></span> <span data-ttu-id="c5fef-431">Het script moet bestaan op de lokale computer of in een map waartoe de lokale computer toegang heeft.</span><span class="sxs-lookup"><span data-stu-id="c5fef-431">The script must exist on the local computer or in a directory that the local computer can access.</span></span> <span data-ttu-id="c5fef-432">Gebruik **argument List** om de waarden van para meters in het script op te geven.</span><span class="sxs-lookup"><span data-stu-id="c5fef-432">Use **ArgumentList** to specify the values of parameters in the script.</span></span>

<span data-ttu-id="c5fef-433">Wanneer u deze para meter gebruikt, wordt de inhoud van het opgegeven script bestand door Power shell naar een script blok geconverteerd, wordt het script blok verzonden naar de externe computer en uitgevoerd op de externe computer.</span><span class="sxs-lookup"><span data-stu-id="c5fef-433">When you use this parameter, PowerShell converts the contents of the specified script file to a script block, transmits the script block to the remote computer, and runs it on the remote computer.</span></span>
...
### <span data-ttu-id="c5fef-434">-HideComputerName</span><span class="sxs-lookup"><span data-stu-id="c5fef-434">-HideComputerName</span></span>

<span data-ttu-id="c5fef-435">Geeft aan dat met deze cmdlet de computer naam van elk object uit de uitvoer weergave wordt wegge laten.</span><span class="sxs-lookup"><span data-stu-id="c5fef-435">Indicates that this cmdlet omits the computer name of each object from the output display.</span></span> <span data-ttu-id="c5fef-436">De naam van de computer die het object heeft gegenereerd, wordt standaard weer gegeven in de weer gave.</span><span class="sxs-lookup"><span data-stu-id="c5fef-436">By default, the name of the computer that generated the object appears in the display.</span></span>

<span data-ttu-id="c5fef-437">Deze para meter is alleen van invloed op de uitvoer weergave.</span><span class="sxs-lookup"><span data-stu-id="c5fef-437">This parameter affects only the output display.</span></span> <span data-ttu-id="c5fef-438">Het object wordt niet gewijzigd.</span><span class="sxs-lookup"><span data-stu-id="c5fef-438">It doesn't change the object.</span></span>
...
### <span data-ttu-id="c5fef-439">-Hostnaam</span><span class="sxs-lookup"><span data-stu-id="c5fef-439">-HostName</span></span>

<span data-ttu-id="c5fef-440">Hiermee geeft u een matrix van computer namen voor een verbinding op basis van SSH (Secure Shell).</span><span class="sxs-lookup"><span data-stu-id="c5fef-440">Specifies an array of computer names for a Secure Shell (SSH) based connection.</span></span> <span data-ttu-id="c5fef-441">Dit is vergelijkbaar met de para meter **ComputerName** , behalve dat de verbinding met de externe computer wordt gemaakt met behulp van SSH in plaats van Windows WinRM.</span><span class="sxs-lookup"><span data-stu-id="c5fef-441">This is similar to the **ComputerName** parameter except that the connection to the remote computer is made using SSH rather than Windows WinRM.</span></span>

<span data-ttu-id="c5fef-442">Deze para meter is geïntroduceerd in Power shell 6,0.</span><span class="sxs-lookup"><span data-stu-id="c5fef-442">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="c5fef-443">-InDisconnectedSession</span><span class="sxs-lookup"><span data-stu-id="c5fef-443">-InDisconnectedSession</span></span>

<span data-ttu-id="c5fef-444">Geeft aan dat met deze cmdlet een opdracht of script wordt uitgevoerd in een sessie waarbij de verbinding is verbroken.</span><span class="sxs-lookup"><span data-stu-id="c5fef-444">Indicates that this cmdlet runs a command or script in a disconnected session.</span></span>

<span data-ttu-id="c5fef-445">Wanneer u de para meter **InDisconnectedSession** gebruikt, `Invoke-Command` wordt op elke externe computer een permanente sessie gemaakt, wordt de opdracht gestart die is opgegeven door de para meter **script Block** of **filepath** en wordt de verbinding met de sessie verbroken.</span><span class="sxs-lookup"><span data-stu-id="c5fef-445">When you use the **InDisconnectedSession** parameter, `Invoke-Command` creates a persistent session on each remote computer, starts the command specified by the **ScriptBlock** or **FilePath** parameter, and then disconnects from the session.</span></span> <span data-ttu-id="c5fef-446">De opdrachten blijven worden uitgevoerd in de verbroken sessies.</span><span class="sxs-lookup"><span data-stu-id="c5fef-446">The commands continue to run in the disconnected sessions.</span></span> <span data-ttu-id="c5fef-447">Met **InDisconnectedSession** kunt u opdrachten uitvoeren zonder een verbinding met de externe sessies te onderhouden.</span><span class="sxs-lookup"><span data-stu-id="c5fef-447">**InDisconnectedSession** enables you to run commands without maintaining a connection to the remote sessions.</span></span> <span data-ttu-id="c5fef-448">En omdat de verbinding van de sessie wordt verbroken voordat er resultaten worden geretourneerd, zorgt **InDisconnectedSession** ervoor dat alle opdracht resultaten worden geretourneerd naar de opnieuw verbonden sessie, in plaats van te worden gesplitst tussen sessies.</span><span class="sxs-lookup"><span data-stu-id="c5fef-448">And, because the session is disconnected before any results are returned, **InDisconnectedSession** makes sure that all command results are returned to the reconnected session, instead of being split between sessions.</span></span>

<span data-ttu-id="c5fef-449">U kunt **InDisconnectedSession** niet gebruiken met de para meter **Session** of de para meter **AsJob** .</span><span class="sxs-lookup"><span data-stu-id="c5fef-449">You can't use **InDisconnectedSession** with the **Session** parameter or the **AsJob** parameter.</span></span>

<span data-ttu-id="c5fef-450">Opdrachten die gebruikmaken van **InDisconnectedSession** retour neren een **PSSession** -object dat de niet-verbonden sessie vertegenwoordigt.</span><span class="sxs-lookup"><span data-stu-id="c5fef-450">Commands that use **InDisconnectedSession** return a **PSSession** object that represents the disconnected session.</span></span> <span data-ttu-id="c5fef-451">Ze retour neren niet de uitvoer van de opdracht.</span><span class="sxs-lookup"><span data-stu-id="c5fef-451">They don't return the command output.</span></span> <span data-ttu-id="c5fef-452">Gebruik de cmdlets of om verbinding te maken met de verbroken sessie `Connect-PSSession` `Receive-PSSession` .</span><span class="sxs-lookup"><span data-stu-id="c5fef-452">To connect to the disconnected session, use the `Connect-PSSession` or `Receive-PSSession` cmdlets.</span></span> <span data-ttu-id="c5fef-453">Gebruik de cmdlet om de resultaten op te halen van de opdrachten die in de sessie worden uitgevoerd `Receive-PSSession` .</span><span class="sxs-lookup"><span data-stu-id="c5fef-453">To get the results of commands that ran in the session, use the `Receive-PSSession` cmdlet.</span></span> <span data-ttu-id="c5fef-454">Om opdrachten uit te voeren die uitvoer genereren in een verbroken sessie, stelt u de waarde van de **OutputBufferingMode** -sessie optie in op **Drop**.</span><span class="sxs-lookup"><span data-stu-id="c5fef-454">To run commands that generate output in a disconnected session, set the value of the **OutputBufferingMode** session option to **Drop**.</span></span> <span data-ttu-id="c5fef-455">Als u van plan bent om verbinding te maken met de verbroken sessie, stelt u de time-out voor inactiviteit in de sessie zo in dat u voldoende tijd hebt om verbinding te maken voordat u de sessie verwijdert.</span><span class="sxs-lookup"><span data-stu-id="c5fef-455">If you intend to connect to the disconnected session, set the idle time-out in the session so that it provides sufficient time for you to connect before deleting the session.</span></span>

<span data-ttu-id="c5fef-456">U kunt de uitvoer buffer modus en time-out voor inactiviteit instellen in de para meter **SessionOption** of in de `$PSSessionOption` Voorkeurs variabele.</span><span class="sxs-lookup"><span data-stu-id="c5fef-456">You can set the output buffering mode and idle time-out in the **SessionOption** parameter or in the `$PSSessionOption` preference variable.</span></span> <span data-ttu-id="c5fef-457">Zie en about_Preference_Variables voor meer informatie over sessie `New-PSSessionOption` opties [about_Preference_Variables](./about/about_preference_variables.md).</span><span class="sxs-lookup"><span data-stu-id="c5fef-457">For more information about session options, see `New-PSSessionOption` and [about_Preference_Variables](./about/about_preference_variables.md).</span></span>

<span data-ttu-id="c5fef-458">Zie [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md)voor meer informatie over de functie voor verbroken sessies.</span><span class="sxs-lookup"><span data-stu-id="c5fef-458">For more information about the Disconnected Sessions feature, see [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md).</span></span>

<span data-ttu-id="c5fef-459">Deze para meter is geïntroduceerd in Power Shell 3,0.</span><span class="sxs-lookup"><span data-stu-id="c5fef-459">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="c5fef-460">-Input object</span><span class="sxs-lookup"><span data-stu-id="c5fef-460">-InputObject</span></span>

<span data-ttu-id="c5fef-461">Hiermee geeft u de invoer aan de opdracht.</span><span class="sxs-lookup"><span data-stu-id="c5fef-461">Specifies input to the command.</span></span> <span data-ttu-id="c5fef-462">Voer een variabele in die de objecten bevat of typ een opdracht of expressie waarmee de objecten worden opgehaald.</span><span class="sxs-lookup"><span data-stu-id="c5fef-462">Enter a variable that contains the objects or type a command or expression that gets the objects.</span></span>

<span data-ttu-id="c5fef-463">Gebruik bij het gebruik van de para meter **input object** de `$Input` Automatische variabele in de waarde van de para meter **script Block** om de invoer objecten weer te geven.</span><span class="sxs-lookup"><span data-stu-id="c5fef-463">When using the **InputObject** parameter, use the `$Input` automatic variable in the value of the **ScriptBlock** parameter to represent the input objects.</span></span>
...
### <span data-ttu-id="c5fef-464">-JobName</span><span class="sxs-lookup"><span data-stu-id="c5fef-464">-JobName</span></span>

<span data-ttu-id="c5fef-465">Hiermee geeft u een beschrijvende naam op voor de achtergrond taak.</span><span class="sxs-lookup"><span data-stu-id="c5fef-465">Specifies a friendly name for the background job.</span></span> <span data-ttu-id="c5fef-466">Taken worden standaard benoemd `Job<n>` , waarbij `<n>` een rang nummer is.</span><span class="sxs-lookup"><span data-stu-id="c5fef-466">By default, jobs are named `Job<n>`, where `<n>` is an ordinal number.</span></span>

<span data-ttu-id="c5fef-467">Als u de para meter **JobName** in een opdracht gebruikt, wordt de opdracht uitgevoerd als een taak en `Invoke-Command` wordt een taak object geretourneerd, zelfs als u geen **AsJob** in de opdracht opneemt.</span><span class="sxs-lookup"><span data-stu-id="c5fef-467">If you use the **JobName** parameter in a command, the command is run as a job, and `Invoke-Command` returns a job object, even if you don't include **AsJob** in the command.</span></span>

<span data-ttu-id="c5fef-468">Zie [about_Jobs](./About/about_Jobs.md)voor meer informatie over Power shell-achtergrond taken.</span><span class="sxs-lookup"><span data-stu-id="c5fef-468">For more information about PowerShell background jobs, see [about_Jobs](./About/about_Jobs.md).</span></span>
...
### <span data-ttu-id="c5fef-469">-Bestandspad</span><span class="sxs-lookup"><span data-stu-id="c5fef-469">-KeyFilePath</span></span>

<span data-ttu-id="c5fef-470">Hiermee geeft u een pad naar een sleutel bestand dat door een Secure Shell (SSH) wordt gebruikt om een gebruiker op een externe computer te verifiëren.</span><span class="sxs-lookup"><span data-stu-id="c5fef-470">Specifies a key file path used by Secure Shell (SSH) to authenticate a user on a remote computer.</span></span>

<span data-ttu-id="c5fef-471">Met SSH kan gebruikers verificatie worden uitgevoerd via persoonlijke en open bare sleutels als alternatief voor de basis wachtwoord verificatie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-471">SSH allows user authentication to be performed via private and public keys as an alternative to basic password authentication.</span></span> <span data-ttu-id="c5fef-472">Als de externe computer is geconfigureerd voor sleutel verificatie, kan deze para meter worden gebruikt om de sleutel op te geven waarmee de gebruiker wordt geïdentificeerd.</span><span class="sxs-lookup"><span data-stu-id="c5fef-472">If the remote computer is configured for key authentication, then this parameter can be used to provide the key that identifies the user.</span></span>

<span data-ttu-id="c5fef-473">Deze para meter is geïntroduceerd in Power shell 6,0.</span><span class="sxs-lookup"><span data-stu-id="c5fef-473">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="c5fef-474">-NoNewScope</span><span class="sxs-lookup"><span data-stu-id="c5fef-474">-NoNewScope</span></span>

<span data-ttu-id="c5fef-475">Geeft aan dat met deze cmdlet de opgegeven opdracht wordt uitgevoerd in het huidige bereik.</span><span class="sxs-lookup"><span data-stu-id="c5fef-475">Indicates that this cmdlet runs the specified command in the current scope.</span></span> <span data-ttu-id="c5fef-476">`Invoke-Command`Voert standaard opdrachten uit in hun eigen bereik.</span><span class="sxs-lookup"><span data-stu-id="c5fef-476">By default, `Invoke-Command` runs commands in their own scope.</span></span>

<span data-ttu-id="c5fef-477">Deze para meter is alleen geldig in opdrachten die worden uitgevoerd in de huidige sessie, dat wil zeggen opdrachten die de **computer naam** en de **sessie** parameters weglaten.</span><span class="sxs-lookup"><span data-stu-id="c5fef-477">This parameter is valid only in commands that are run in the current session, that is, commands that omit both the **ComputerName** and **Session** parameters.</span></span>

<span data-ttu-id="c5fef-478">Deze para meter is geïntroduceerd in Power Shell 3,0.</span><span class="sxs-lookup"><span data-stu-id="c5fef-478">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="c5fef-479">-Port</span><span class="sxs-lookup"><span data-stu-id="c5fef-479">-Port</span></span>

<span data-ttu-id="c5fef-480">Hiermee geeft u de netwerk poort op de externe computer die wordt gebruikt voor deze opdracht.</span><span class="sxs-lookup"><span data-stu-id="c5fef-480">Specifies the network port on the remote computer that is used for this command.</span></span> <span data-ttu-id="c5fef-481">Als u verbinding wilt maken met een externe computer, moet op de externe computer worden geluisterd op de poort die door de verbinding wordt gebruikt.</span><span class="sxs-lookup"><span data-stu-id="c5fef-481">To connect to a remote computer, the remote computer must be listening on the port that the connection uses.</span></span> <span data-ttu-id="c5fef-482">De standaard poorten zijn 5985 (WinRM-poort voor HTTP) en 5986 (WinRM-poort voor HTTPS).</span><span class="sxs-lookup"><span data-stu-id="c5fef-482">The default ports are 5985 (WinRM port for HTTP) and 5986 (WinRM port for HTTPS).</span></span>

<span data-ttu-id="c5fef-483">Voordat u een andere poort gebruikt, configureert u de WinRM-listener op de externe computer om op die poort te Luis teren.</span><span class="sxs-lookup"><span data-stu-id="c5fef-483">Before using an alternate port, configure the WinRM listener on the remote computer to listen at that port.</span></span> <span data-ttu-id="c5fef-484">Als u de listener wilt configureren, typt u de volgende twee opdrachten bij de Power shell-prompt:</span><span class="sxs-lookup"><span data-stu-id="c5fef-484">To configure the listener, type the following two commands at the PowerShell prompt:</span></span>

`Remove-Item -Path WSMan:\Localhost\listener\listener* -Recurse`

`New-Item -Path WSMan:\Localhost\listener -Transport http -Address * -Port \<port-number\>`

<span data-ttu-id="c5fef-485">Gebruik de para meter **poort** alleen als dat nodig is.</span><span class="sxs-lookup"><span data-stu-id="c5fef-485">Don't use the **Port** parameter unless you must.</span></span> <span data-ttu-id="c5fef-486">De poort die in de opdracht is ingesteld, is van toepassing op alle computers of sessies waarop de opdracht wordt uitgevoerd.</span><span class="sxs-lookup"><span data-stu-id="c5fef-486">The port that is set in the command applies to all computers or sessions on which the command runs.</span></span> <span data-ttu-id="c5fef-487">Een alternatieve poort instelling kan verhinderen dat de opdracht wordt uitgevoerd op alle computers.</span><span class="sxs-lookup"><span data-stu-id="c5fef-487">An alternate port setting might prevent the command from running on all computers.</span></span>
...
### <span data-ttu-id="c5fef-488">-RemoteDebug</span><span class="sxs-lookup"><span data-stu-id="c5fef-488">-RemoteDebug</span></span>

<span data-ttu-id="c5fef-489">Wordt gebruikt om de aangeroepen opdracht in de foutopsporingsmodus in de externe Power shell-sessie uit te voeren.</span><span class="sxs-lookup"><span data-stu-id="c5fef-489">Used to run the invoked command in debug mode in the remote PowerShell session.</span></span>
...
### <span data-ttu-id="c5fef-490">-RunAsAdministrator</span><span class="sxs-lookup"><span data-stu-id="c5fef-490">-RunAsAdministrator</span></span>

<span data-ttu-id="c5fef-491">Geeft aan dat deze cmdlet een opdracht aanroept als beheerder.</span><span class="sxs-lookup"><span data-stu-id="c5fef-491">Indicates that this cmdlet invokes a command as an Administrator.</span></span>
...
### <span data-ttu-id="c5fef-492">-Script Block</span><span class="sxs-lookup"><span data-stu-id="c5fef-492">-ScriptBlock</span></span>

<span data-ttu-id="c5fef-493">Hiermee geeft u de opdrachten op die moeten worden uitgevoerd.</span><span class="sxs-lookup"><span data-stu-id="c5fef-493">Specifies the commands to run.</span></span> <span data-ttu-id="c5fef-494">Plaats de opdrachten tussen accolades `{ }` om een script blok te maken.</span><span class="sxs-lookup"><span data-stu-id="c5fef-494">Enclose the commands in curly braces `{ }` to create a script block.</span></span>
<span data-ttu-id="c5fef-495">Deze parameter is vereist.</span><span class="sxs-lookup"><span data-stu-id="c5fef-495">This parameter is required.</span></span>

<span data-ttu-id="c5fef-496">Standaard worden alle variabelen in de opdracht geëvalueerd op de externe computer.</span><span class="sxs-lookup"><span data-stu-id="c5fef-496">By default, any variables in the command are evaluated on the remote computer.</span></span> <span data-ttu-id="c5fef-497">Als u lokale variabelen wilt toevoegen aan de opdracht, gebruikt u **argument List**.</span><span class="sxs-lookup"><span data-stu-id="c5fef-497">To include local variables in the command, use **ArgumentList**.</span></span>
...
### <span data-ttu-id="c5fef-498">-Sessie</span><span class="sxs-lookup"><span data-stu-id="c5fef-498">-Session</span></span>

<span data-ttu-id="c5fef-499">Hiermee geeft u een matrix met sessies waarin met deze cmdlet de opdracht wordt uitgevoerd.</span><span class="sxs-lookup"><span data-stu-id="c5fef-499">Specifies an array of sessions in which this cmdlet runs the command.</span></span> <span data-ttu-id="c5fef-500">Voer een variabele in die **PSSession** -objecten bevat of een opdracht waarmee de **PSSession** -objecten, zoals een of-opdracht, worden gemaakt of opgehaald `New-PSSession` `Get-PSSession` .</span><span class="sxs-lookup"><span data-stu-id="c5fef-500">Enter a variable that contains **PSSession** objects or a command that creates or gets the **PSSession** objects, such as a `New-PSSession` or `Get-PSSession` command.</span></span>

<span data-ttu-id="c5fef-501">Wanneer u een **PSSession** maakt, brengt Power shell een permanente verbinding met de externe computer tot stand.</span><span class="sxs-lookup"><span data-stu-id="c5fef-501">When you create a **PSSession** , PowerShell establishes a persistent connection to the remote computer.</span></span> <span data-ttu-id="c5fef-502">Gebruik een **PSSession** om een reeks gerelateerde opdrachten uit te voeren die gegevens delen.</span><span class="sxs-lookup"><span data-stu-id="c5fef-502">Use a **PSSession** to run a series of related commands that share data.</span></span> <span data-ttu-id="c5fef-503">Als u één opdracht of een reeks niet-gerelateerde opdrachten wilt uitvoeren, gebruikt u de para meter **ComputerName** .</span><span class="sxs-lookup"><span data-stu-id="c5fef-503">To run a single command or a series of unrelated commands, use the **ComputerName** parameter.</span></span> <span data-ttu-id="c5fef-504">Zie [about_PSSessions](./About/about_PSSessions.md)voor meer informatie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-504">For more information, see [about_PSSessions](./About/about_PSSessions.md).</span></span>
...
### <span data-ttu-id="c5fef-505">-Sessie naam</span><span class="sxs-lookup"><span data-stu-id="c5fef-505">-SessionName</span></span>

<span data-ttu-id="c5fef-506">Hiermee geeft u een beschrijvende naam op voor een niet-verbonden sessie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-506">Specifies a friendly name for a disconnected session.</span></span> <span data-ttu-id="c5fef-507">U kunt de naam gebruiken om te verwijzen naar de sessie in volgende opdrachten, zoals een `Get-PSSession` opdracht.</span><span class="sxs-lookup"><span data-stu-id="c5fef-507">You can use the name to refer to the session in subsequent commands, such as a `Get-PSSession` command.</span></span> <span data-ttu-id="c5fef-508">Deze para meter is alleen geldig voor de para meter **InDisconnectedSession** .</span><span class="sxs-lookup"><span data-stu-id="c5fef-508">This parameter is valid only with the **InDisconnectedSession** parameter.</span></span>

<span data-ttu-id="c5fef-509">Deze para meter is geïntroduceerd in Power Shell 3,0.</span><span class="sxs-lookup"><span data-stu-id="c5fef-509">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="c5fef-510">-SessionOption</span><span class="sxs-lookup"><span data-stu-id="c5fef-510">-SessionOption</span></span>

<span data-ttu-id="c5fef-511">Hiermee geeft u geavanceerde opties voor de sessie op.</span><span class="sxs-lookup"><span data-stu-id="c5fef-511">Specifies advanced options for the session.</span></span> <span data-ttu-id="c5fef-512">Voer een **SessionOption** -object in, zoals het account dat u maakt met behulp van de `New-PSSessionOption` cmdlet of een hash-tabel waarin de sleutels de namen van sessie opties zijn en de waarden zijn waarden voor de sessie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-512">Enter a **SessionOption** object, such as one that you create using the `New-PSSessionOption` cmdlet, or a hash table in which the keys are session option names and the values are session option values.</span></span>

<span data-ttu-id="c5fef-513">De standaard waarden voor de opties worden bepaald door de waarde van de `$PSSessionOption` Voorkeurs variabele, als deze is ingesteld.</span><span class="sxs-lookup"><span data-stu-id="c5fef-513">The default values for the options are determined by the value of the `$PSSessionOption` preference variable, if it's set.</span></span> <span data-ttu-id="c5fef-514">Anders worden de standaard waarden bepaald door opties die zijn ingesteld in de sessie configuratie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-514">Otherwise, the default values are established by options set in the session configuration.</span></span>

<span data-ttu-id="c5fef-515">De waarden van de sessie optie hebben voor rang op de standaard waarden voor sessies die zijn ingesteld in de `$PSSessionOption` Voorkeurs variabele en in de sessie configuratie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-515">The session option values take precedence over default values for sessions set in the `$PSSessionOption` preference variable and in the session configuration.</span></span> <span data-ttu-id="c5fef-516">Ze hebben echter geen voor rang op de maximum waarden, quota's of limieten die zijn ingesteld in de sessie configuratie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-516">However, they don't take precedence over maximum values, quotas, or limits set in the session configuration.</span></span>

<span data-ttu-id="c5fef-517">Zie voor een beschrijving van de sessie opties die de standaard waarden bevatten `New-PSSessionOption` .</span><span class="sxs-lookup"><span data-stu-id="c5fef-517">For a description of the session options that includes the default values, see `New-PSSessionOption`.</span></span> <span data-ttu-id="c5fef-518">Zie about_Preference_Variables voor meer informatie over de `$PSSessionOption` voorkeurs [about_Preference_Variables](About/about_Preference_Variables.md)variabele.</span><span class="sxs-lookup"><span data-stu-id="c5fef-518">For information about the `$PSSessionOption` preference variable, see [about_Preference_Variables](About/about_Preference_Variables.md).</span></span>
<span data-ttu-id="c5fef-519">Zie [about_Session_Configurations](About/about_Session_Configurations.md) (Engelstalig) voor meer informatie over sessieconfiguraties.</span><span class="sxs-lookup"><span data-stu-id="c5fef-519">For more information about session configurations, see [about_Session_Configurations](About/about_Session_Configurations.md).</span></span>
...
### <span data-ttu-id="c5fef-520">-SSHConnection</span><span class="sxs-lookup"><span data-stu-id="c5fef-520">-SSHConnection</span></span>

<span data-ttu-id="c5fef-521">Deze para meter gebruikt een matrix van hash-tabellen waarbij elke hash-tabel een of meer verbindings parameters bevat die nodig zijn om een SSH-verbinding (Secure Shell) tot stand te brengen.</span><span class="sxs-lookup"><span data-stu-id="c5fef-521">This parameter takes an array of hash tables where each hash table contains one or more connection parameters needed to establish a Secure Shell (SSH) connection.</span></span> <span data-ttu-id="c5fef-522">De **SSHConnection** -para meter is handig voor het maken van meerdere sessies waarbij voor elke sessie verschillende verbindings gegevens zijn vereist.</span><span class="sxs-lookup"><span data-stu-id="c5fef-522">The **SSHConnection** parameter is useful for creating multiple sessions where each session requires different connection information.</span></span>

<span data-ttu-id="c5fef-523">De hashtabel bevat de volgende leden:</span><span class="sxs-lookup"><span data-stu-id="c5fef-523">The hashtable has the following members:</span></span>

- <span data-ttu-id="c5fef-524">**ComputerName** (of **hostnaam** )</span><span class="sxs-lookup"><span data-stu-id="c5fef-524">**ComputerName** (or **HostName** )</span></span>
- <span data-ttu-id="c5fef-525">**Poort**</span><span class="sxs-lookup"><span data-stu-id="c5fef-525">**Port**</span></span>
- <span data-ttu-id="c5fef-526">**Gebruikers**</span><span class="sxs-lookup"><span data-stu-id="c5fef-526">**UserName**</span></span>
- <span data-ttu-id="c5fef-527">Het **bestandspad** (of **IdentityFilePath** )</span><span class="sxs-lookup"><span data-stu-id="c5fef-527">**KeyFilePath** (or **IdentityFilePath** )</span></span>

<span data-ttu-id="c5fef-528">**ComputerName** (of **hostnaam** ) is het enige sleutel-waardepaar dat vereist is.</span><span class="sxs-lookup"><span data-stu-id="c5fef-528">**ComputerName** (or **HostName** ) is the only key-value pair that is required.</span></span>

<span data-ttu-id="c5fef-529">Deze para meter is geïntroduceerd in Power shell 6,0.</span><span class="sxs-lookup"><span data-stu-id="c5fef-529">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="c5fef-530">-SSHTransport</span><span class="sxs-lookup"><span data-stu-id="c5fef-530">-SSHTransport</span></span>

<span data-ttu-id="c5fef-531">Geeft aan dat de externe verbinding tot stand is gebracht met behulp van Secure Shell (SSH).</span><span class="sxs-lookup"><span data-stu-id="c5fef-531">Indicates that the remote connection is established using Secure Shell (SSH).</span></span>

<span data-ttu-id="c5fef-532">Power Shell maakt standaard gebruik van Windows WinRM om verbinding te maken met een externe computer.</span><span class="sxs-lookup"><span data-stu-id="c5fef-532">By default PowerShell uses Windows WinRM to connect to a remote computer.</span></span> <span data-ttu-id="c5fef-533">Met deze switch wordt Power shell gedwongen de para meter **hostname** te gebruiken om een externe verbinding op basis van SSH tot stand te brengen.</span><span class="sxs-lookup"><span data-stu-id="c5fef-533">This switch forces PowerShell to use the **HostName** parameter for establishing an SSH based remote connection.</span></span>

<span data-ttu-id="c5fef-534">Deze para meter is geïntroduceerd in Power shell 6,0.</span><span class="sxs-lookup"><span data-stu-id="c5fef-534">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="c5fef-535">-ThrottleLimit</span><span class="sxs-lookup"><span data-stu-id="c5fef-535">-ThrottleLimit</span></span>

<span data-ttu-id="c5fef-536">Hiermee geeft u het maximum aantal gelijktijdige verbindingen op dat tot stand kan worden gebracht om deze opdracht uit te voeren.</span><span class="sxs-lookup"><span data-stu-id="c5fef-536">Specifies the maximum number of concurrent connections that can be established to run this command.</span></span>
<span data-ttu-id="c5fef-537">Als u deze para meter weglaat of een waarde van 0 invoert, wordt de standaard waarde 32 gebruikt.</span><span class="sxs-lookup"><span data-stu-id="c5fef-537">If you omit this parameter or enter a value of 0, the default value, 32, is used.</span></span>

<span data-ttu-id="c5fef-538">De beperkings limiet geldt alleen voor de huidige opdracht, niet voor de sessie of voor de computer.</span><span class="sxs-lookup"><span data-stu-id="c5fef-538">The throttle limit applies only to the current command, not to the session or to the computer.</span></span>
...
### <span data-ttu-id="c5fef-539">-GebruikersNaam</span><span class="sxs-lookup"><span data-stu-id="c5fef-539">-UserName</span></span>

<span data-ttu-id="c5fef-540">Hiermee geeft u de gebruikers naam op voor het account dat wordt gebruikt om een opdracht uit te voeren op de externe computer.</span><span class="sxs-lookup"><span data-stu-id="c5fef-540">Specifies the username for the account used to run a command on the remote computer.</span></span> <span data-ttu-id="c5fef-541">De verificatie methode voor de gebruiker is afhankelijk van hoe Secure Shell (SSH) is geconfigureerd op de externe computer.</span><span class="sxs-lookup"><span data-stu-id="c5fef-541">The user authentication method depends on how Secure Shell (SSH) is configured on the remote computer.</span></span>

<span data-ttu-id="c5fef-542">Als SSH is geconfigureerd voor basis wachtwoord verificatie, wordt u gevraagd om het wacht woord van de gebruiker.</span><span class="sxs-lookup"><span data-stu-id="c5fef-542">If SSH is configured for basic password authentication, then you'll be prompted for the user password.</span></span>

<span data-ttu-id="c5fef-543">Als SSH is geconfigureerd voor gebruikers authenticatie op basis van sleutels, kan een pad naar een sleutel bestand worden opgegeven **via de para** meter voor het sleutelpad en wordt er geen wachtwoord prompt weer gegeven.</span><span class="sxs-lookup"><span data-stu-id="c5fef-543">If SSH is configured for key-based user authentication then a key file path can be provided via the **KeyFilePath** parameter and no password prompt occurs.</span></span> <span data-ttu-id="c5fef-544">Als het sleutel bestand van de client gebruiker zich in een bekende SSH-locatie bevindt, is de para meter voor het sleutelpad niet nodig voor verificatie op basis van een sleutel en **wordt de gebruikers** verificatie automatisch uitgevoerd op basis van de gebruikers naam.</span><span class="sxs-lookup"><span data-stu-id="c5fef-544">If the client user key file is located in an SSH known location, then the **KeyFilePath** parameter isn't needed for key-based authentication, and user authentication occurs automatically based on the username.</span></span> <span data-ttu-id="c5fef-545">Zie de SSH-documentatie van uw platform over gebruikers verificatie op basis van een sleutel voor meer informatie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-545">For more information, see your platform's SSH documentation about key-based user authentication.</span></span>

<span data-ttu-id="c5fef-546">Dit is geen vereiste para meter.</span><span class="sxs-lookup"><span data-stu-id="c5fef-546">This isn't a required parameter.</span></span> <span data-ttu-id="c5fef-547">Als de para meter **username** niet is opgegeven, wordt de huidige aangemelde gebruikers naam gebruikt voor de verbinding.</span><span class="sxs-lookup"><span data-stu-id="c5fef-547">If the **UserName** parameter isn't specified, then the current logged on username is used for the connection.</span></span>

<span data-ttu-id="c5fef-548">Deze para meter is geïntroduceerd in Power shell 6,0.</span><span class="sxs-lookup"><span data-stu-id="c5fef-548">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="c5fef-549">-UseSSL</span><span class="sxs-lookup"><span data-stu-id="c5fef-549">-UseSSL</span></span>

<span data-ttu-id="c5fef-550">Geeft aan dat deze cmdlet het Secure Sockets Layer-Protocol (SSL) gebruikt om verbinding te maken met de externe computer.</span><span class="sxs-lookup"><span data-stu-id="c5fef-550">Indicates that this cmdlet uses the Secure Sockets Layer (SSL) protocol to establish a connection to the remote computer.</span></span> <span data-ttu-id="c5fef-551">SSL wordt standaard niet gebruikt.</span><span class="sxs-lookup"><span data-stu-id="c5fef-551">By default, SSL isn't used.</span></span>

<span data-ttu-id="c5fef-552">WS-Management versleutelt alle Power shell-inhoud die via het netwerk wordt verzonden.</span><span class="sxs-lookup"><span data-stu-id="c5fef-552">WS-Management encrypts all PowerShell content transmitted over the network.</span></span> <span data-ttu-id="c5fef-553">De para meter **UseSSL** is een extra beveiliging waarbij de gegevens worden verzonden via een HTTPS, in plaats van via http.</span><span class="sxs-lookup"><span data-stu-id="c5fef-553">The **UseSSL** parameter is an additional protection that sends the data across an HTTPS, instead of HTTP.</span></span>

<span data-ttu-id="c5fef-554">Als u deze para meter gebruikt, maar SSL niet beschikbaar is op de poort die wordt gebruikt voor de opdracht, mislukt de opdracht.</span><span class="sxs-lookup"><span data-stu-id="c5fef-554">If you use this parameter, but SSL isn't available on the port that's used for the command, the command fails.</span></span>
...
### <span data-ttu-id="c5fef-555">-VMId</span><span class="sxs-lookup"><span data-stu-id="c5fef-555">-VMId</span></span>

<span data-ttu-id="c5fef-556">Hiermee geeft u een matrix met Id's van virtuele machines op.</span><span class="sxs-lookup"><span data-stu-id="c5fef-556">Specifies an array of IDs of virtual machines.</span></span>
...
### <span data-ttu-id="c5fef-557">-VMName</span><span class="sxs-lookup"><span data-stu-id="c5fef-557">-VMName</span></span>

<span data-ttu-id="c5fef-558">Hiermee geeft u een matrix met namen van virtuele machines op.</span><span class="sxs-lookup"><span data-stu-id="c5fef-558">Specifies an array of names of virtual machines.</span></span>
...
### <span data-ttu-id="c5fef-559">-Subsysteem</span><span class="sxs-lookup"><span data-stu-id="c5fef-559">-Subsystem</span></span>

<span data-ttu-id="c5fef-560">Hiermee geeft u het SSH-subsysteem op dat wordt gebruikt voor de nieuwe **PSSession**.</span><span class="sxs-lookup"><span data-stu-id="c5fef-560">Specifies the SSH subsystem used for the new **PSSession**.</span></span>

<span data-ttu-id="c5fef-561">Hiermee geeft u het subsysteem op dat moet worden gebruikt op het doel zoals gedefinieerd in sshd_config.</span><span class="sxs-lookup"><span data-stu-id="c5fef-561">This specifies the subsystem to use on the target as defined in sshd_config.</span></span>
<span data-ttu-id="c5fef-562">Het subsysteem start een specifieke versie van Power shell met vooraf gedefinieerde para meters.</span><span class="sxs-lookup"><span data-stu-id="c5fef-562">The subsystem starts a specific version of PowerShell with predefined parameters.</span></span>
<span data-ttu-id="c5fef-563">Als het opgegeven subsysteem niet bestaat op de externe computer, mislukt de opdracht.</span><span class="sxs-lookup"><span data-stu-id="c5fef-563">If the specified subsystem does not exist on the remote computer, the command fails.</span></span>

<span data-ttu-id="c5fef-564">Als deze para meter niet wordt gebruikt, is de standaard het subsysteem Power shell.</span><span class="sxs-lookup"><span data-stu-id="c5fef-564">If this parameter is not used, the default is the 'powershell' subsystem.</span></span>
...
### <span data-ttu-id="c5fef-565">CommonParameters</span><span class="sxs-lookup"><span data-stu-id="c5fef-565">CommonParameters</span></span>

<span data-ttu-id="c5fef-566">Deze cmdlet biedt ondersteuning voor de meest gebruikte parameters: -Debug, - ErrorAction, - ErrorVariable, - InformationAction, -InformationVariable, - OutVariable,-OutBuffer, - PipelineVariable - Verbose, - WarningAction en -WarningVariable.</span><span class="sxs-lookup"><span data-stu-id="c5fef-566">This cmdlet supports the common parameters: -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction, and -WarningVariable.</span></span> <span data-ttu-id="c5fef-567">Zie [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216)voor meer informatie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-567">For more information, see [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216).</span></span>

## <span data-ttu-id="c5fef-568">INVOER</span><span class="sxs-lookup"><span data-stu-id="c5fef-568">INPUTS</span></span>

### <span data-ttu-id="c5fef-569">System. Management. Automation. script Block</span><span class="sxs-lookup"><span data-stu-id="c5fef-569">System.Management.Automation.ScriptBlock</span></span>

<span data-ttu-id="c5fef-570">U kunt een opdracht in een script blok door sluizen naar `Invoke-Command` .</span><span class="sxs-lookup"><span data-stu-id="c5fef-570">You can pipe a command in a script block to `Invoke-Command`.</span></span> <span data-ttu-id="c5fef-571">Gebruik de `$Input` Automatische variabele om de invoer objecten in de opdracht weer te geven.</span><span class="sxs-lookup"><span data-stu-id="c5fef-571">Use the `$Input` automatic variable to represent the input objects in the command.</span></span>

## <span data-ttu-id="c5fef-572">UITVOER</span><span class="sxs-lookup"><span data-stu-id="c5fef-572">OUTPUTS</span></span>

### <span data-ttu-id="c5fef-573">System. Management. Automation. PSRemotingJob, System. Management. Automation. Runspaces. PSSession, of de uitvoer van de aangeroepen opdracht</span><span class="sxs-lookup"><span data-stu-id="c5fef-573">System.Management.Automation.PSRemotingJob, System.Management.Automation.Runspaces.PSSession, or the output of the invoked command</span></span>

<span data-ttu-id="c5fef-574">Met deze cmdlet wordt een taak object geretourneerd als u de para meter **AsJob** gebruikt.</span><span class="sxs-lookup"><span data-stu-id="c5fef-574">This cmdlet returns a job object, if you use the **AsJob** parameter.</span></span> <span data-ttu-id="c5fef-575">Als u de para meter **InDisconnectedSession** opgeeft, `Invoke-Command` wordt een **PSSession** -object geretourneerd.</span><span class="sxs-lookup"><span data-stu-id="c5fef-575">If you specify the **InDisconnectedSession** parameter, `Invoke-Command` returns a **PSSession** object.</span></span> <span data-ttu-id="c5fef-576">Anders wordt de uitvoer van de aangeroepen opdracht geretourneerd. Dit is de waarde van de para meter **script Block** .</span><span class="sxs-lookup"><span data-stu-id="c5fef-576">Otherwise, it returns the output of the invoked command, which is the value of the **ScriptBlock** parameter.</span></span>

## <span data-ttu-id="c5fef-577">OPMERKINGEN</span><span class="sxs-lookup"><span data-stu-id="c5fef-577">NOTES</span></span>

<span data-ttu-id="c5fef-578">In Windows Vista en latere versies van het Windows-besturings systeem **ComputerName** `Invoke-Command` moet u Power shell uitvoeren met de optie **als administrator uitvoeren** om de para meter ComputerName van te gebruiken voor het uitvoeren van een opdracht op de lokale computer.</span><span class="sxs-lookup"><span data-stu-id="c5fef-578">On Windows Vista, and later versions of the Windows operating system, to use the **ComputerName** parameter of `Invoke-Command` to run a command on the local computer, you must run PowerShell using the **Run as administrator** option.</span></span>

<span data-ttu-id="c5fef-579">Wanneer u opdrachten op meerdere computers uitvoert, maakt Power shell verbinding met de computers in de volg orde waarin ze worden weer gegeven in de lijst.</span><span class="sxs-lookup"><span data-stu-id="c5fef-579">When you run commands on multiple computers, PowerShell connects to the computers in the order in which they appear in the list.</span></span> <span data-ttu-id="c5fef-580">De uitvoer van de opdracht wordt echter weer gegeven in de volg orde waarin deze worden ontvangen van de externe computers. Dit kan anders zijn.</span><span class="sxs-lookup"><span data-stu-id="c5fef-580">However, the command output is displayed in the order that it's received from the remote computers, which might be different.</span></span>

<span data-ttu-id="c5fef-581">Fouten die het resultaat zijn van de opdracht die `Invoke-Command` wordt uitgevoerd, worden opgenomen in de opdracht resultaten.</span><span class="sxs-lookup"><span data-stu-id="c5fef-581">Errors that result from the command that `Invoke-Command` runs are included in the command results.</span></span>
<span data-ttu-id="c5fef-582">Fouten waarvoor fouten in een lokale opdracht zouden worden beëindigd, worden beschouwd als niet-afsluit fouten in een externe opdracht.</span><span class="sxs-lookup"><span data-stu-id="c5fef-582">Errors that would be terminating errors in a local command are treated as non-terminating errors in a remote command.</span></span> <span data-ttu-id="c5fef-583">Deze strategie zorgt ervoor dat het beëindigen van fouten op één computer de opdracht niet sluit op alle computers waarop deze wordt uitgevoerd.</span><span class="sxs-lookup"><span data-stu-id="c5fef-583">This strategy makes sure that terminating errors on one computer don't close the command on all computers on which it's run.</span></span> <span data-ttu-id="c5fef-584">Deze procedure wordt ook gebruikt wanneer een externe opdracht wordt uitgevoerd op één computer.</span><span class="sxs-lookup"><span data-stu-id="c5fef-584">This practice is used even when a remote command is run on a single computer.</span></span>

<span data-ttu-id="c5fef-585">Als de externe computer zich niet in een domein bevindt dat de lokale computer vertrouwt, is de computer mogelijk niet in staat om de referenties van de gebruiker te verifiëren.</span><span class="sxs-lookup"><span data-stu-id="c5fef-585">If the remote computer isn't in a domain that the local computer trusts, the computer might not be able to authenticate the user's credentials.</span></span> <span data-ttu-id="c5fef-586">Als u de externe computer wilt toevoegen aan de lijst met vertrouwde hosts in WS-Management, gebruikt u de volgende opdracht in de `WSMAN` provider, waarbij `<Remote-Computer-Name>` de naam is van de externe computer:</span><span class="sxs-lookup"><span data-stu-id="c5fef-586">To add the remote computer to the list of trusted hosts in WS-Management, use the following command in the `WSMAN` provider, where `<Remote-Computer-Name>` is the name of the remote computer:</span></span>

`Set-Item -Path WSMan:\Localhost\Client\TrustedHosts -Value \<Remote-Computer-Name\>`

<span data-ttu-id="c5fef-587">Wanneer u een **PSSession** verbreekt met de para meter **InDisconnectedSession** , wordt de sessie status **losgekoppeld** en is de beschik baarheid **geen**.</span><span class="sxs-lookup"><span data-stu-id="c5fef-587">When you disconnect a **PSSession** using the **InDisconnectedSession** parameter, the session state is **Disconnected** and the availability is **None**.</span></span> <span data-ttu-id="c5fef-588">De waarde van de eigenschap **State** is relatief ten opzichte van de huidige sessie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-588">The value of the **State** property is relative to the current session.</span></span> <span data-ttu-id="c5fef-589">Als de waarde voor de **verbinding is verbroken** , betekent dit dat de **PSSession** niet is verbonden met de huidige sessie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-589">A value of **Disconnected** means that the **PSSession** isn't connected to the current session.</span></span> <span data-ttu-id="c5fef-590">Dit betekent echter niet dat de **PSSession** van de verbinding met alle sessies is verbroken.</span><span class="sxs-lookup"><span data-stu-id="c5fef-590">However, it doesn't mean that the **PSSession** is disconnected from all sessions.</span></span> <span data-ttu-id="c5fef-591">Deze is mogelijk verbonden met een andere sessie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-591">It might be connected to a different session.</span></span> <span data-ttu-id="c5fef-592">Gebruik de eigenschap **Beschik baarheid** om te bepalen of u verbinding kunt maken met de sessie of er opnieuw verbinding mee wilt maken.</span><span class="sxs-lookup"><span data-stu-id="c5fef-592">To determine whether you can connect or reconnect to the session, use the **Availability** property.</span></span>

<span data-ttu-id="c5fef-593">Een **beschikbaarheids** waarde van **geen** geeft aan dat u verbinding kunt maken met de sessie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-593">An **Availability** value of **None** indicates that you can connect to the session.</span></span> <span data-ttu-id="c5fef-594">De waarde **bezet** geeft aan dat u geen verbinding kunt maken met de **PSSession** , omdat deze is verbonden met een andere sessie.</span><span class="sxs-lookup"><span data-stu-id="c5fef-594">A value of **Busy** indicates that you can't connect to the **PSSession** because it's connected to another session.</span></span> <span data-ttu-id="c5fef-595">Zie [RunspaceState](/dotnet/api/system.management.automation.runspaces.runspacestate)voor meer informatie over de waarden van de eigenschap **State** van sessies.</span><span class="sxs-lookup"><span data-stu-id="c5fef-595">For more information about the values of the **State** property of sessions, see [RunspaceState](/dotnet/api/system.management.automation.runspaces.runspacestate).</span></span> <span data-ttu-id="c5fef-596">Zie [RunspaceAvailability](/dotnet/api/system.management.automation.runspaces.runspaceavailability)voor meer informatie over de waarden van de eigenschap **Beschik baarheid** van sessies.</span><span class="sxs-lookup"><span data-stu-id="c5fef-596">For more information about the values of the **Availability** property of sessions, see [RunspaceAvailability](/dotnet/api/system.management.automation.runspaces.runspaceavailability).</span></span>

<span data-ttu-id="c5fef-597">De para meters **hostname** en **SSHConnection** zijn opgenomen vanaf Power shell 6,0.</span><span class="sxs-lookup"><span data-stu-id="c5fef-597">The **HostName** and **SSHConnection** parameters were included starting with PowerShell 6.0.</span></span> <span data-ttu-id="c5fef-598">Ze zijn toegevoegd om Power shell Remoting te bieden op basis van de Secure Shell (SSH).</span><span class="sxs-lookup"><span data-stu-id="c5fef-598">They were added to provide PowerShell remoting based on Secure Shell (SSH).</span></span> <span data-ttu-id="c5fef-599">Power shell en SSH worden ondersteund op meerdere platformen (Windows, Linux, macOS) en externe communicatie van Power shell werkt op deze platforms waarin Power shell en SSH zijn geïnstalleerd en geconfigureerd.</span><span class="sxs-lookup"><span data-stu-id="c5fef-599">PowerShell and SSH are supported on multiple platforms (Windows, Linux, macOS) and PowerShell remoting works over these platforms where PowerShell and SSH are installed and configured.</span></span> <span data-ttu-id="c5fef-600">Dit is gescheiden van de vorige Windows-functie voor externe toegang die is gebaseerd op WinRM en veel van de specifieke WinRM-functies en-beperkingen zijn niet van toepassing.</span><span class="sxs-lookup"><span data-stu-id="c5fef-600">This is separate from the previous Windows only remoting that is based on WinRM and many of the WinRM specific features and limitations don't apply.</span></span> <span data-ttu-id="c5fef-601">Bijvoorbeeld: op WinRM gebaseerde quota's, sessie opties, aangepaste eindpunt configuratie en functies voor ontkoppelen/opnieuw verbinden worden op dit moment niet ondersteund.</span><span class="sxs-lookup"><span data-stu-id="c5fef-601">For example WinRM based quotas, session options, custom endpoint configuration, and disconnect/reconnect features are currently not supported.</span></span> <span data-ttu-id="c5fef-602">Zie voor meer informatie over het instellen van externe toegang tot Power shell [via SSH Power shell](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).</span><span class="sxs-lookup"><span data-stu-id="c5fef-602">For more information about how to set up PowerShell SSH remoting, see [PowerShell Remoting Over SSH](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).</span></span>

## <span data-ttu-id="c5fef-603">GERELATEERDE KOPPELINGEN</span><span class="sxs-lookup"><span data-stu-id="c5fef-603">RELATED LINKS</span></span>

[<span data-ttu-id="c5fef-604">about_PSSessions</span><span class="sxs-lookup"><span data-stu-id="c5fef-604">about_PSSessions</span></span>](./About/about_PSSessions.md)

[<span data-ttu-id="c5fef-605">about_Remote</span><span class="sxs-lookup"><span data-stu-id="c5fef-605">about_Remote</span></span>](./About/about_Remote.md)

[<span data-ttu-id="c5fef-606">about_Remote_Disconnected_Sessions</span><span class="sxs-lookup"><span data-stu-id="c5fef-606">about_Remote_Disconnected_Sessions</span></span>](./About/about_Remote_Disconnected_Sessions.md)

[<span data-ttu-id="c5fef-607">about_Remote_Troubleshooting</span><span class="sxs-lookup"><span data-stu-id="c5fef-607">about_Remote_Troubleshooting</span></span>](./About/about_remote_troubleshooting.md)

[<span data-ttu-id="c5fef-608">about_Remote_Variables</span><span class="sxs-lookup"><span data-stu-id="c5fef-608">about_Remote_Variables</span></span>](./About/about_Remote_Variables.md)

[<span data-ttu-id="c5fef-609">about_Scopes</span><span class="sxs-lookup"><span data-stu-id="c5fef-609">about_Scopes</span></span>](./About/about_scopes.md)

[<span data-ttu-id="c5fef-610">Enter-PSSession</span><span class="sxs-lookup"><span data-stu-id="c5fef-610">Enter-PSSession</span></span>](Enter-PSSession.md)

[<span data-ttu-id="c5fef-611">Afsluiten-PSSession</span><span class="sxs-lookup"><span data-stu-id="c5fef-611">Exit-PSSession</span></span>](Exit-PSSession.md)

[<span data-ttu-id="c5fef-612">Get-PSSession</span><span class="sxs-lookup"><span data-stu-id="c5fef-612">Get-PSSession</span></span>](Get-PSSession.md)

[<span data-ttu-id="c5fef-613">Invoke-item</span><span class="sxs-lookup"><span data-stu-id="c5fef-613">Invoke-Item</span></span>](../Microsoft.PowerShell.Management/Invoke-Item.md)

[<span data-ttu-id="c5fef-614">New-PSSession</span><span class="sxs-lookup"><span data-stu-id="c5fef-614">New-PSSession</span></span>](New-PSSession.md)

[<span data-ttu-id="c5fef-615">Remove-PSSession</span><span class="sxs-lookup"><span data-stu-id="c5fef-615">Remove-PSSession</span></span>](Remove-PSSession.md)

[<span data-ttu-id="c5fef-616">WSMan Provider</span><span class="sxs-lookup"><span data-stu-id="c5fef-616">WSMan Provider</span></span>](../Microsoft.WsMan.Management/About/about_WSMan_Provider.md)
