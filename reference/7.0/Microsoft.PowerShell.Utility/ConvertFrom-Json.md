...
### Voor beeld 5: een matrix met één element retour ronding

Met deze opdracht wordt een voor beeld weer gegeven waarin de `-NoEnumerate` Schakel optie wordt gebruikt voor het afronden van een JSON-matrix met één element.
...
De JSON-teken reeks bevat een matrix met één element. Zonder de switch, het converteren van de JSON naar een PSObject en het opnieuw converteren met de `ConvertTo-Json` opdracht resulteert in één geheel getal.
...
Hiermee geeft u op dat de uitvoer niet wordt opgesomd.

Als deze para meter wordt ingesteld, worden matrices als één object verzonden, in plaats van dat elk element afzonderlijk wordt verzonden. Dit garandeert dat JSON kan worden afgerond via `ConvertTo-Json` .
