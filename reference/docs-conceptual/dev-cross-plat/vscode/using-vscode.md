...
1. Installeer de Power shell-extensie.
   ...
   1. Start de app Visual Studio code door te typen `code` in een-console of `code-insiders` Als u Visual Studio code insiders hebt geïnstalleerd.
   1. Start **snel openen** in Windows of Linux door op <kbd>CTRL</kbd> + <kbd>P</kbd>te drukken. Druk op macOS op <kbd>cmd</kbd> + <kbd>P</kbd>.
   1. Typ in snel openen `ext install powershell` en druk op **Enter**.
...
Na het lezen van de documentatie kunt u configuratie-instellingen toevoegen in `settings.json` .
...
Als u niet wilt dat deze instellingen van invloed zijn op alle bestands typen, kunt u met Visual Studio code ook configuraties per taal instellen. Maak een taalspecifieke instelling door instellingen in een veld in te voeren `[<language-name>]` . Bijvoorbeeld:
...
U kunt andere uitvoer bare Power shell-paden toevoegen aan het menu sessie met behulp van de [Visual Studio code-instelling](https://code.visualstudio.com/docs/getstarted/settings): `powershell.powerShellAdditionalExePaths` .
...
- `exePath`: Het pad naar het `pwsh` of het `powershell` uitvoer bare bestand.
- `versionName`: De tekst die wordt weer gegeven in het menu van de sessie.

Als u de standaard-Power shell-versie wilt instellen, stelt u de waarde `powershell.powerShellDefaultVersion` in op de tekst die wordt weer gegeven in het menu sessie (ook wel bekend als de `versionName` ):
...
Met Visual Studio code maakt u een map en een bestand `.vscode\launch.json` in de hoofdmap van de map werk ruimte voor het opslaan van de configuratie van fout opsporing. Als uw bestanden zich in een Git-opslag plaats bevinden, wilt u het bestand doorgaans door voeren `launch.json` . De inhoud van het `launch.json` bestand is:
