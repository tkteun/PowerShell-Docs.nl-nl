...
Gebruik de cmdlet `Get-Content` om de inhoud van het opgegeven item in de database tabel op te halen in het pad dat is opgegeven door de para meter `Path`. Met de para meter `ReadCount` wordt het aantal items opgegeven dat door de gedefinieerde inhouds lezer moet worden gelezen (standaard 1). Met de volgende opdracht wordt de cmdlet twee rijen (items) opgehaald uit de tabel en wordt de inhoud ervan weer gegeven. Houd er rekening mee dat in de volgende voorbeeld uitvoer een fictieve Access-Data Base wordt gebruikt.
