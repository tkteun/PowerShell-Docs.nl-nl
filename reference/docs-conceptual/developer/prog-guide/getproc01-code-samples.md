...
Hier volgen de code voorbeelden voor de voor beeld-cmdlet GetProc01. Dit is het basis voorbeeld van het `Get-Process`-cmdlet dat wordt beschreven in [uw eerste cmdlet maken](../cmdlet/creating-a-cmdlet-without-parameters.md). Een `Get-Process`-cmdlet is ontworpen voor het ophalen van informatie over alle processen die worden uitgevoerd op de lokale computer.
