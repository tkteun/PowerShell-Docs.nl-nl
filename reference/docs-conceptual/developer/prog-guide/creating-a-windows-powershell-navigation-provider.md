...
De voor beeld-navigatie provider overschrijft deze methode niet. De standaard implementatie wordt hieronder weer gegeven. Het accepteert paden die zowel "/" als "\\" als padscheidingsteken gebruiken. Eerst wordt het pad genormaliseerd met alleen ' \\ ' scheidings tekens, vervolgens wordt het bovenliggende pad gesplitst bij de laatste ' \\ ' en wordt de naam van het onderliggende padcomponent geretourneerd.
...
3. Gebruik de cmdlet `Set-Location` opnieuw om de locatie van de gegevens tabel werk nemers in te stellen.
...
7. Gebruik de cmdlet `Get-Item` opnieuw om de werknemers gegevens voor de items in rij 0 op te halen.
