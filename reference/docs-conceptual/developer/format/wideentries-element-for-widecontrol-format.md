...
In het volgende voor beeld ziet u een `WideEntries`-element dat één `WideEntry`-element definieert. Het element `WideEntry` bevat één element `WideItem` waarmee wordt gedefinieerd welke eigenschap of script waarde wordt weer gegeven in de weer gave.
