...
Het gebruik van de Windows Power shell-webservice vereist dat een derde partij de interface [micro soft. Management. Odata. CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) implementeert om Windows Power shell-cmdlets beschikbaar te maken. Deze interface voert de gebruikers autorisatie uit voor de webservice. Nadat u de code hebt geschreven om de interface te implementeren, moet u deze compileren in een DLL-bestand dat moet worden gebruikt in de webtoepassing.
...
In het volgende voor beeld wordt een klasse gemaakt die het autorisatie beleid parseert in het XML-bestand.
...
```
 ...
 De volgende klassen vertegenwoordigen respectievelijk groepen en gebruikers.
