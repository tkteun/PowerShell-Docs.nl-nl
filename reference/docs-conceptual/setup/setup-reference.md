---
ms.date: 2017-06-05
keywords: PowerShell-cmdlet
title: Naslaggids voor Setup
ms.openlocfilehash: 39da5f2b524d908270066926ff2cdc56a665fe35
ms.sourcegitcommit: 18e3bfae83ffe282d3fd1a45f5386f3b7250f0c0
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 02/08/2018
---
# <a name="setup-guide"></a>Handleiding voor het instellen

- [PowerShell Core installeert op Mac OS- en Linux](Installing-PowerShell-Core-on-macOS-and-Linux.md)
- [PowerShell Core installeren in Windows](Installing-PowerShell-Core-on-Windows.md)
- [Windows PowerShell installeren](Installing-Windows-PowerShell.md)
- [De Windows PowerShell 2.0-engine installeren](Installing-the-Windows-PowerShell-2.0-Engine.md)
- [De Windows PowerShell SDK installeren](Installing-the-Windows-PowerShell-SDK.md)
- [Naslaggids voor Setup](setup-reference.md)
- [Toegankelijkheid in Windows PowerShell ISE](Accessibility-in-Windows-PowerShell-ISE.md)
- [De 32-bits versie van Windows PowerShell starten](Starting-the-32-Bit-Version-of-Windows-PowerShell.md)
- [De Windows PowerShell 2.0-engine starten](Starting-the-Windows-PowerShell-2.0-Engine.md)
- [Windows PowerShell op oudere versies van Windows wordt gestart](Starting-Windows-PowerShell-on-Earlier-Versions-of-Windows.md)
- [Windows PowerShell starten](Starting-Windows-PowerShell.md)
- [Systeemvereisten voor Windows PowerShell](Windows-PowerShell-System-Requirements.md)
- [WinRMSecurity](WinRMSecurity.md)
