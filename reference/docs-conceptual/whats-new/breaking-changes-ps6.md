...
Als gevolg van het gebruik van niet-ondersteunde Api's, `*-EventLog` is het verwijderd uit Power shell core. totdat er een betere oplossing is gevonden. `Get-WinEvent` en `New-WinEvent` zijn beschikbaar voor het ophalen en maken van gebeurtenissen in Windows.
...
### <a name="add-get-stringhash-and-get-filehash-cmdlets-3024"></a>`Get-StringHash`#3024 toevoegen en `Get-FileHash` cmdlets [](https://github.com/PowerShell/PowerShell/issues/3024)
