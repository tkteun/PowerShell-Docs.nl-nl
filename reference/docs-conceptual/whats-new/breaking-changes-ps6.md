...
### <a name="rename-powershellexe-to-pwshexe-5101"></a>Naam `powershell.exe` wijzigen `pwsh.exe` in [#5101](https://github.com/PowerShell/PowerShell/issues/5101)
...
### <a name="change-outputencoding-to-use-utf-8-nobom-encoding-rather-than-ascii-5369"></a>Wijziging `$OutputEncoding` van het `UTF-8 NoBOM` gebruik van code ring in plaats van ASCII- [#5369](https://github.com/PowerShell/PowerShell/issues/5369)
...
### <a name="-verbose-and--debug-no-longer-overrides-erroractionpreference-5113"></a>`-Verbose`en `-Debug` worden niet meer overschreven `$ErrorActionPreference` [#5113](https://github.com/PowerShell/PowerShell/issues/5113)
...
### <a name="import-csv-should-apply-pstypenames-upon-import-when-type-information-is-present-in-the-csv-5134"></a>`Import-Csv`moet worden `PSTypeNames` toegepast bij het importeren wanneer type gegevens aanwezig zijn in het CSV- [#5134](https://github.com/PowerShell/PowerShell/issues/5134)

Voorheen behouden objecten die `Export-CSV` `TypeInformation` `ConvertFrom-Csv` zijn geëxporteerd met met met geïmporteerd met niet de type-informatie. Met deze wijziging worden de type gegevens `PSTypeNames` aan het lid toegevoegd als deze beschikbaar zijn vanuit het CSV-bestand.
...
### <a name="web-cmdlets-should-warn-when--credential-is-sent-over-unencrypted-connections-5112"></a>Web-cmdlets moeten worden `-Credential` gewaarschuwd wanneer wordt verzonden via niet-versleutelde verbindingen [#5112](https://github.com/PowerShell/PowerShell/issues/5112)
...
### <a name="remove-addtypecommandbase-class-5407"></a>#5407 `AddTypeCommandBase` klasse [#5407](https://github.com/PowerShell/PowerShell/issues/5407) verwijderen
...
### <a name="unify-cmdlets-with-parameter--encoding-to-be-of-type-systemtextencoding-5080"></a>De cmdlets worden samen `-Encoding` met para meters `System.Text.Encoding` van het type [#5080](https://github.com/PowerShell/PowerShell/issues/5080)
...
### <a name="add-better-error-message-for-empty-and-null--uformat-parameter-5055"></a>Een beter fout bericht toevoegen voor lege en `-UFormat` null-para meter [#5055](https://github.com/PowerShell/PowerShell/issues/5055)
...
### <a name="removed-runspaceconfiguration-support-4942"></a>Ondersteunings `RunspaceConfiguration` [#4942](https://github.com/PowerShell/PowerShell/issues/4942) verwijderd
...
### <a name="commandinvocationintrinsicsinvokescript-bind-arguments-to-input-instead-of-args-4923"></a>`CommandInvocationIntrinsics.InvokeScript`argumenten binden aan `$input` in plaats `$args` van [#4923](https://github.com/PowerShell/PowerShell/issues/4923)
...
### <a name="fix-set-service-failing-test-4802"></a>Mislukte `Set-Service` test [#4802](https://github.com/PowerShell/PowerShell/issues/4802) herstellen
...
### <a name="rename-isosx-to-ismacos-4700"></a>Naam `$IsOSX` wijzigen `$IsMacOS` in [#4700](https://github.com/PowerShell/PowerShell/issues/4700)
...
### <a name="removal-of-localaccount-and-cmdlets-from--diagnostics-modules-4302-4303"></a>Het verwijderen `LocalAccount` van en cmdlets `Diagnostics` uit modules. [#4302](https://github.com/PowerShell/PowerShell/issues/4302) [#4303](https://github.com/PowerShell/PowerShell/issues/4303)
...
Voorheen werd het gebruik `-File` van **Power shell. exe** (nu **pwsh. exe**) gebruikt om een Power shell-script uit `$true` / `$false` te voeren dat is ingesteld op geen enkele manier als parameter waarden worden door gegeven. Er is `$true` / `$false` ondersteuning toegevoegd voor als geparseerde waarden voor para meters. Switch waarden worden ook ondersteund, omdat de syntaxis die momenteel wordt beschreven, niet werkt.

### <a name="remove-clrversion-property-from-psversiontable-4027"></a>Eigenschap `ClrVersion` verwijderen uit `$PSVersionTable` [#4027](https://github.com/PowerShell/PowerShell/issues/4027)
...
### <a name="change-positional-parameter-for-powershellexe-from--command-to--file-4019"></a>Positie `powershell.exe` parameters voor wijzigen `-Command` van `-File` in [#4019](https://github.com/PowerShell/PowerShell/issues/4019)

Schakel het shebang-gebruik van Power shell in op niet-Windows-platforms. Dit betekent dat u op UNIX gebaseerde systemen een script bestand kunt maken dat Power shell automatisch aanroept in plaats van expliciet `pwsh`aan te roepen. Dit betekent ook dat u nu dingen kunt doen zoals `powershell foo.ps1` of `powershell fooScript` zonder op `-File`te geven. Voor deze wijziging moet u echter expliciet opgeven `-c` of `-Command` als u wilt proberen om dingen te doen `powershell.exe Get-Command`.
...
`` `u####``of `` `u{####}`` wordt geconverteerd naar het bijbehorende Unicode-teken. Als u een letterlijke waarde `` `u``wilt uitvoeren, ``` ``u```sluit u de apostroffen:.

### <a name="change-new-modulemanifest-encoding-to-utf8nobom-on-non-windows-platforms-3940"></a>Wijzig `New-ModuleManifest` de code `UTF8NoBOM` ring in op niet-Windows-platforms [#3940](https://github.com/PowerShell/PowerShell/issues/3940)

Voorheen `New-ModuleManifest` maakt psd1-manifesten in UTF-16 met stuk lijst, waardoor er een probleem is met Linux-hulpprogram ma's. Door deze wijziging wordt de code ring `New-ModuleManifest` van als UTF (geen bom) gewijzigd in niet-Windows-platforms.
...
### <a name="add-get-stringhash-and-get-filehash-cmdlets-3024"></a>#3024 `Get-StringHash` toevoegen `Get-FileHash` en cmdlets [#3024](https://github.com/PowerShell/PowerShell/issues/3024)
...
### <a name="parameter-binding-problem-with-valuefromremainingarguments-in-ps-functions-2035"></a>Fout bij het binden `ValueFromRemainingArguments` van de para meter met in PS-functies [#2035](https://github.com/PowerShell/PowerShell/issues/2035)
