...
- **Waar ()** operator gedrag is gewijzigd. `Collection.Where('property -match name')`het accepteren van een teken reeks expressie `"Property -CompareOperator Value"` in de indeling wordt niet meer ondersteund.
...
U kunt werk stromen schrijven in XAML of in de Windows Power shell-taal en uitvoeren op dezelfde manier als u een cmdlet zou uitvoeren. De `Get-Command` cmdlet krijgt werk stroom opdrachten en `Get-Help` de cmdlet krijgt hulp voor werk stromen.
...
U kunt nu bijgewerkte Help-bestanden voor de cmdlets in uw modules downloaden. Met `Update-Help` de cmdlet worden de nieuwste Help-bestanden geïdentificeerd, worden deze gedownload van Internet, worden ze uitgepakt, gevalideerd en worden ze in de juiste taalspecifieke Directory voor de module geïnstalleerd.
...
Ter ondersteuning van gebruikers die geen toegang hebben tot internet en gebruikers achter firewalls `Save-Help` , downloadt de nieuwe cmdlet Help-bestanden naar een bestandssysteem Directory, zoals een bestands share. Gebruikers kunnen de `Update-Help` cmdlet vervolgens gebruiken om bijgewerkte Help-bestanden van de bestands share op te halen.
...
Windows 8-en Windows Server 2012-modules bevatten geen Help-bestanden. Als u de nieuwste Help-bestanden wilt `Update-Help`downloaden, typt u. Typ `Get-Help` (zonder para meters) of Zie [about_Updatable_Help](/powershell/module/microsoft.powershell.core/about/about_Updatable_Help)voor meer informatie.

Wanneer de Help bestanden voor een cmdlet niet op de computer zijn geïnstalleerd, wordt `Get-Help` in de cmdlet nu automatisch gegenereerde Help weer gegeven. De automatisch gegenereerde Help omvat de opdracht syntaxis en instructies voor het gebruik `Update-Help` van de cmdlet om Help-bestanden te downloaden.
...
Gebruik de `New-PSSessionConfigurationFile` cmdlet om een sessie configuratie bestand te maken. Gebruik de `Register-PSSessionConfiguration` of ' set-PSSessionConfiguration-cmdlets om het sessie configuratie bestand toe te passen op een sessie configuratie.
...
Het automatisch importeren van modules wordt geactiveerd met behulp van de cmdlet in een opdracht `Get-Command` die wordt uitgevoerd voor een cmdlet zonder joker tekens `Get-Help` of die wordt uitgevoerd voor een cmdlet zonder joker tekens.
...
Zie [about_Modules](/powershell/module/microsoft.powershell.core/about/about_modules), [about_Preference_Variables](/powershell/module/microsoft.powershell.core/about/about_Preference_Variables)en de Help-onderwerpen voor de `Get-Command` `Import-Module` cmdlets voor meer informatie.
...
7. Stillere, gestroomlijnde werking van `New-ModuleManifest` de cmdlet.
...
Voor het verbeteren van de mogelijkheden van Windows Power Shell 3,0 het interpreteren en goed verwerken van speciale tekens, is de para meter **LiteralPath** , die speciale tekens in paden verwerkt, geldig op bijna alle cmdlets die de `Update-Help` para `Save-Help` meter **Path** hebben, inclusief de nieuwe en cmdlets. De parser bevat ook speciale logica voor het verbeteren van de verwerking van het\`apostroffen teken () en vier Kante haken in bestands namen en paden.
