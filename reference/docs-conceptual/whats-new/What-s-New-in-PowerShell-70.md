...
Controleer de installatie-instructies voor uw favoriete besturings systeem [Windows](/powershell/scripting/install/installing-powershell-core-on-windows), [macOS](/powershell/scripting/install/installing-powershell-core-on-macos)of [Linux](/powershell/scripting/install/installing-powershell-core-on-linux).
...
> [!NOTE]
> Debian 10 en CentOS 8 bieden momenteel geen ondersteuning voor externe toegang via WinRM. Zie voor meer informatie over het instellen van op SSH gebaseerde externe communicatie [Power shell voor externe toegang via SSH](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).
...
Voor meer informatie [over als](/powershell/module/microsoft.powershell.core/about/about_if).
...
Voor meer informatie [over pijplijn keten operators](/powershell/module/microsoft.powershell.core/about/about_pipeline_chain_operators?view=powershell-7&preserve-view=true).
...
> [!NOTE]
> Dit is een experimentele functie met de naam **PSNullConditionalOperators**. Zie [experimentele functies gebruiken](/powershell/scripting/learn/experimental-features)voor meer informatie.
...
[About_Operators](/powershell/module/microsoft.powershell.core/about/about_operators?view=powershell-7&preserve-view=true)voor meer informatie.

## <a name="new-view-conciseview-and-cmdlet-get-error"></a>Nieuwe weer gave-ConciseView en cmdlet-Get-Error
...
Voor meer informatie [over meldingen over updates](/powershell/module/microsoft.powershell.core/about/about_update_notifications).
...
> [!NOTE]
> Dit is een experimentele functie met de naam **PSDesiredStateConfiguration. InvokeDscResource**. Zie [experimentele functies gebruiken](/powershell/scripting/learn/experimental-features)voor meer informatie.
...
Voor meer informatie over [invoke-dscresource bieden](/powershell/module/psdesiredstateconfiguration/invoke-dscresource?view=powershell-7&preserve-view=true).
...
- Werk Test-Connection om meer te doen als in Windows Power shell (#10697) (bedankt @vexx32 !)
...
- Add-AsPlainText naar ConvertFrom-SecureString (#11142)
...
- Regressie in Get-PSCallStack corrigeren (#11210) (bedankt @iSazonov !)
- Autoloading van de ScheduledJob-module verwijderen bij het gebruik van taak-cmdlets (#11194)
- Output type toevoegen aan Get-Error cmdlet en oorspronkelijke typenames behouden (#10856)
- Herstel de null-verwijzing in de eigenschap SupportsVirtualTerminal (#11105)
- Controle van limiet toevoegen in Get-WinEvent (#10648) (bedankt @iSazonov !)
...
- Para meter Culture toevoegen aan Select-String cmdlet (#10943) (bedankt @iSazonov !)
- Start-Job werkmap met afsluitende back slash (#11041) herstellen
- ConvertFrom-JSON: verzamelingen standaard uitpakken (#10861) (bedankt @danstur !)
- Gebruik een hoofdletter gevoelige hashtabel voor Group-Object cmdlet met de Schakel opties-CaseSensitive en-AsHashtable #11030 (bedankt @vexx32 !)
...
- De installatie van meer dan één CommandInfo met Get-Command (#10929) herstellen
- Get-Counter-cmdlet voor Windows toevoegen (#10933)
...
- Get-Clipboard voor meerdere platformen en Set-Clipboard (#10340) toegevoegd
...
- Een Out-Printer-opdracht toevoegen in Windows (#10906)
- Start-Job-variabele workingdirectory oplossen met een spatie (#10951)
...
- GraphicalHost-assembly toevoegen om-GridView, show-Command en Get-Help-ShowWindow (#10899) in te scha kelen
...
- Een Update-List-opdracht toevoegen (#10922)
- FWLink-id voor Clear-RecycleBin bijwerken (#10925)
- Bij het volt ooien van het tabblad bestand overs Laan als bestands kenmerken niet kunnen worden gelezen (#10910)
- Een back-Clear-RecycleBin voor Windows toevoegen (#10909)
- Voeg toe `$env:__SuppressAnsiEscapeSequences` om te bepalen of er een escape-teken reeks moet worden uitgevoerd (#10814)
- De para meter add-all voor het inkleuren van Select-String uitvoer (#8963) (bedankt @derek-xia !)
- De cmdlet back Get-HotFix toevoegen (#10740)
- Add-Type bruikbaar maken in toepassingen die Power shell hosten (#10587)
...
- Fix Get-Content-ReadCount 0-gedrag wanneer-TotalCount is ingesteld (#10749) (bedankt @eugenesmlv !)
- Fout bericht over het weigeren van de toegang tot woorden in Get-WinEvent (#10639) (bedankt @iSazonov !)
...
- De para meter voor het toevoegen van een scheidings teken voor ConvertFrom-StringData (#10665) (bedankt @steviecoaster !)
- Positionele para meter voor script Block toevoegen bij het gebruik van Invoke-Command met SSH (#10721) (bedankt @machgo !)
...
- Informatieve uitvoer verwijderen, het gebruik van pings in Test-Connection samen voegen (#10478) (bedankt @vexx32 !)
...
- Verwijder [ValidateNotNullOrEmpty] uit-input object op Get-Random om een lege teken reeks toe te staan (#10644)
- Niet-hoofdletter gevoelig (#10549) voor suggestie systeem teken reeks @iSazonov
- Uitzonde ring voor Null-verwijzingen in ForEach-Object-parallelle invoer verwerking (#10577) herstellen
...
- De para meter variabele workingdirectory toevoegen aan Start-Job (#10324) (bedankt @davinci26 !)
...
- De verkeerde uitvoer voor New-Service in een variabele Assignment en-outvariable (#10444) corrigeren (bedankt @kvprasoon !)
...
- Er is een fout opgetreden bij het toevoegen van Add-Type als er al een type met dezelfde naam bestaat (#9609) (bedankt @iSazonov !)
...
- Maak ForEach-Object sneller voor de meestgebruikte scenario's (#10454) en los het probleem op ForEach-Object-parallel prestaties met veel runspaces (#10455)
...
- De niet-werkende Update-Help van Get-Help verwijderen wanneer Get-Help voor de eerste keer wordt uitgevoerd (#10974)
...
- Wijzig de naam van de standaard ParameterSetName in een scheidings teken voor ConvertTo-Csv en ConvertFrom-Csv (#10425)
...
- Get-ChildItem test toevoegen (#10507) (bedankt @iSazonov !)
...
- Build-Info JSON-bestand uploaden met het veld ReleaseDate (#10986)
