...
Nadat de LCM op het doel knooppunt is geconfigureerd en de configuratie documenten zijn gemaakt en op de juiste manier zijn benoemd op de pull-server, haalt het doel knooppunt de gedeeltelijke configuraties op, combineert ze en past de resulterende configuratie op een regel matige wijze toe. intervallen zoals opgegeven door de eigenschap **RefreshFrequencyMins** van de LCM. Als u het vernieuwen wilt forceren, kunt u de cmdlet [Update-DscConfiguration](/powershell/module/PSDesiredStateConfiguration/Update-DscConfiguration) aanroepen om de configuraties op te halen en vervolgens `Start-DSCConfiguration –UseExisting` om ze toe te passen.
