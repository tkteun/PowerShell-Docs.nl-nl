...
_Applies: Windows Power Shell 4,0, Windows Power shell 5.0 @ no__t-0
...
## <a name="my-script-wont-run-using-dsc-logs-to-diagnose-script-errors"></a>Mijn script wordt niet uitgevoerd: DSC-Logboeken gebruiken om script fouten te diagnosticeren
...
In Logboeken bevinden DSC-gebeurtenissen zich in: **Logboeken toepassingen en services/micro soft/Windows/desired state Configuration**
...
### <a name="4-error-messages-logged-for-recent-failed-operations"></a>3 Fout berichten die zijn geregistreerd voor recente mislukte bewerkingen

`$SeparateDscOperations[0].Group` bevat een set gebeurtenissen voor de laatste bewerking. Voer de `Where-Object`-cmdlet uit om de gebeurtenissen te filteren op basis van de weergave naam van het niveau. De resultaten worden opgeslagen in de variabele `$myFailedEvent`, die verder kan worden ontdubbeld om het gebeurtenis bericht op te halen:
...
### <a name="5-all-events-generated-for-a-particular-job-id"></a>5,0 Alle gebeurtenissen die worden gegenereerd voor een bepaalde taak-ID.

`$SeparateDscOperations` is een matrix van groepen, die elk de naam hebben als de unieke taak-ID. Door de `Where-Object`-cmdlet uit te voeren, kunt u deze groepen gebeurtenissen met een bepaalde taak-ID extra heren:
...
De cmdlet `Trace-xDscOperation` retourneert een object met een verzameling gebeurtenissen, hun gebeurtenis typen en de bericht uitvoer die is gegenereerd op basis van een bepaalde DSC-bewerking. Als u een fout ontdekt in een van de bewerkingen met `Get-xDscOperation`, zou u die bewerking traceren om te bepalen welke gebeurtenissen een fout veroorzaken.
...
Als u bijvoorbeeld een `SequenceID` opgeeft, `Trace-xDscOperaion` de tracering ophalen voor de DSC-bewerking die is genegende van de laatste bewerking:
...
Geef de **GUID** door die is toegewezen aan een specifieke DSC-bewerking (zoals geretourneerd door de cmdlet `Get-xDscOperation`) om de gebeurtenis Details voor die DSC-bewerking op te halen:
...
Houd er rekening mee dat, omdat `Trace-xDscOperation` gebeurtenissen uit de logboeken analyse, fout opsporing en operationeel worden verzameld, u wordt gevraagd deze logboeken in te scha kelen, zoals hierboven beschreven.
...
In het ideale geval moet u `Get-xDscOperation` eerst gebruiken om de laatste DSC-configuratie op uw machines uit te geven. Daarna kunt u een enkele bewerking (met behulp van de SequenceID of JobID) met `Trace-xDscOperation` bekijken om te ontdekken wat het achter de schermen heeft gedaan.
...
Hieronder ziet u een demonstratie waarin wordt weer gegeven hoe `DebugMode` automatisch de cache kan vernieuwen. Laten we eerst de standaard configuratie bekijken:
