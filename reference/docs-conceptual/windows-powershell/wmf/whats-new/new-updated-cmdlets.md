...
De **UpdateTemplate** para meter slaat de resultaten van het leer algoritme op in een opmerking in het sjabloon bestand. Dit maakt het leer proces (het langzaamste stadium) één eenmalige kosten. `ConvertFrom-String`Het uitvoeren van een sjabloon die het algoritme voor gecodeerde lessen bevat, is nu bijna direct.
...
In het volgende voor beeld wordt het beste product opgehaald en wordt de uitvoer in de variabele vastgelegd `$infoStream` .
