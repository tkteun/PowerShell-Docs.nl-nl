...
Voor het aanroepen van basis klassen methoden van overschreven implementaties, Casting naar de basis klasse ( `[baseClass]$this` ) op aanroepen:
...
Verborgen leden zijn openbaar, maar worden niet weer gegeven in de uitvoer van `Get-Member` tenzij de `-Force` para meter is toegevoegd. Verborgen leden worden niet opgenomen wanneer het tabblad wordt voltooid of gebruikmaakt van IntelliSense, tenzij de voltooiing plaatsvindt in de klasse die het verborgen lid definieert.
