...
Vanaf Power shell 7,1 kunt u een splatted-para meter onderdrukken door expliciet een para meter in een opdracht te definiëren.
...
### <a name="example-1-reuse-splatted-parameters-in-different-commands"></a>Voor beeld 1: splatted-para meters opnieuw gebruiken in verschillende opdrachten
...
### <a name="example-2-forward-parameters-using-psboundparameters"></a>Voor beeld 2: para meters door sturen met $PSBoundParameters
...
### <a name="example-3-override-splatted-parameters-with-explicitly-defined-parameters"></a>Voor beeld 3: splatted-para meters negeren met expliciet gedefinieerde para meters

In dit voor beeld ziet u hoe u een splatted-para meter overschrijft met expliciet gedefinieerde para meters. Dit is handig als u geen nieuwe hashtabel wilt maken of als u een waarde in de hashtabel wilt wijzigen die u gebruikt voor splat.

De `$commonParams` variabele slaat de para meters op voor het maken van virtuele machines op de `East US` locatie. De `$allVms` variabele is een lijst met virtuele machines die moeten worden gemaakt. De lijst wordt door lopen en gebruikt `$commonParams` om de para meters te splat voor het maken van elke virtuele machine. We willen echter `myVM2` in een andere regio dan de andere virtuele machines worden gemaakt. In plaats van de `$commonParams` hashtabel aan te passen, kunt u expliciet de **locatie** parameter in `New-AzVm` vervangen door de waarde van de `Location` sleutel in `$commonParams` .
