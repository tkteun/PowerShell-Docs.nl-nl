...
## SAMENVATTING
Een experimentele functie uitschakelen bij het opstarten van een nieuw exemplaar van Power shell.
...
`Disable-ExperimentalFeature`Met de cmdlet worden experimentele functies uitgeschakeld door de genoemde experimentele functies te verwijderen uit het `powershell.config.json` instellingen bestand dat in Power shell is gelezen.
...
### Voor beeld 1: een experimentele functie uitschakelen

Als deze experimentele functie eerder was ingeschakeld, wordt het bestand in dit voor beeld `powershell.config.json` bijgewerkt zodat de gebruiker deze functie niet kan inschakelen nadat Power shell opnieuw is opgestart.
...
Pipe-exemplaren van ExperimentalFeature uit de `Get-ExperimentalFeature` cmdlet die moeten worden uitgeschakeld.
