...
De eerste opdracht gebruikt de `Get-ChildItem` cmdlet om het file info-object op te halen `Test.txt`  .
...
De `$this` variabele verwijst naar het exemplaar van het object waarnaar de eigenschappen en methoden worden toegevoegd. Zie about_Automatic_Variables voor meer informatie over de `$this` variabele [](../Microsoft.PowerShell.Core/About/about_Automatic_Variables.md).
