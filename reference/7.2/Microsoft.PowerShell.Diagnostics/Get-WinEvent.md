...
### Voor beeld 18: SuppressHashFilter gebruiken om toepassings fouten te filteren

Net als voor beeld 16 hierboven gebruikt dit voor beeld de para meter **FilterHashtable** voor het ophalen van gebeurtenissen uit het **toepassings** logboek. We voegen echter de **SuppressHashFilter** -sleutel toe om gebeurtenissen op **informatie** niveau uit te filteren.
...
In dit voor beeld worden `Get-WinEvent` alle gebeurtenissen uit het **toepassings** logboek voor de afgelopen twee dagen opgehaald, met uitzonde ring van die met een **niveau** van 4 (informatie).
...
- `<named-data>` sleutel vertegenwoordigt een benoemd gebeurtenis gegevens veld.
...
U kunt ook een **onderdrukkings** element maken met behulp van de para meter **FilterHashtable** .
...
Met de  para meter ListLog `Get-WinEvent` retourneert **System. Diagnostics. eventing. Reader. EventLogConfiguration** -objecten.

Met de  para meter ListProvider `Get-WinEvent` retourneert **System. Diagnostics. eventing. Reader. ProviderMetadata** -objecten.
