...
1. Plaats de `<ProviderName>` map, in dit geval de nuGet-map, op de bijbehorende locatie op de doelcomputer. Als uw doelcomputer een Nano-server is, moet u uitvoeren vanaf Nano Server om `Install-PackageProvider` de juiste binaire NuGet-bestanden te downloaden.
1. Start PowerShell opnieuw op om de pakketprovider automatisch te laden. U kunt ook uitvoeren om `Get-PackageProvider -ListAvailable` alle pakketproviders weer te geven die beschikbaar zijn op de computer.
...
Hiermee geeft u de minimaal toegestane versie van de pakketprovider op die u wilt installeren. Als u deze parameter niet toevoegt, installeert de hoogst beschikbare versie van het pakket dat ook voldoet aan een vereiste die is opgegeven door de `Install-PackageProvider` *parameter MaximumVersion.*
