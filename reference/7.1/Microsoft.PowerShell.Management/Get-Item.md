...
In dit voorbeeld worden items in de Windows-map met namen van een punt ( ) , maar `.` niet met . `w*` Dit voorbeeld werkt alleen als het pad een jokerteken ( ) bevat om de inhoud van het `*` item op te geven.
