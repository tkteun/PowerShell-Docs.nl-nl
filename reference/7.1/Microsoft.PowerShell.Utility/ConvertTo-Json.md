...
Vanaf PowerShell 7.1 geeft een waarschuwing als de diepte van het invoerobject groter is dan de diepte die is `ConvertTo-Json` opgegeven voor de opdracht. Hiermee voorkomt u ongewenst gegevensverlies bij het converteren van objecten.
...
In dit voorbeeld ziet u de uitvoer van `ConvertTo-Json` de cmdlet met en zonder de **asArray-switchparameter.** U kunt zien dat het tweede deel van de uitvoer tussen matrixhaken is verpakt.
...
Hiermee geeft u op hoeveel niveaus van ingesloten objecten zijn opgenomen in de JSON-weergave. De waarde kan elk getal van tot `1` `[Int]::MaxValue` zijn. De standaardwaarde is `2`. `ConvertTo-Json` geeft een waarschuwing weer als het aantal niveaus in een invoerobject dit aantal overschrijdt.
...
Biedt een alternatieve serialisatieoptie die alle enumeraties converteert naar de tekenreeksweergave.
