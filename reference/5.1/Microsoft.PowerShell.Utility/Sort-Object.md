...
Met deze opdracht worden twee eigenschappen gebruikt om de objecten, **status** en **DisplayName** te sorteren. De **status** wordt gesorteerd in aflopende volg orde en **DisplayName** wordt in oplopende volg orde gesorteerd.
...
De `Get-Content` cmdlet gebruikt de para meter **Path** om de map en de bestands naam op te geven. Het bestand **ProductId.txt** bevat een niet-gesorteerde lijst met product nummers. De teken reeks objecten worden naar de-cmdlet verzonden via de pijp lijn `ForEach-Object` . `ForEach-Object` maakt gebruik van een script blok om de teken reeksen te converteren naar gehele getallen. In de voorbeeld code wordt `[int]` de teken reeks geconverteerd naar een geheel getal en `$_` wordt elke teken reeks aangeduid als de pijp lijn. De objecten met gehele getallen worden via de pijp lijn naar de `Sort-Object` cmdlet verzonden. `Sort-Object` de objecten van het gehele getal worden in numerieke volg orde gesorteerd.
## PARAMETERS
