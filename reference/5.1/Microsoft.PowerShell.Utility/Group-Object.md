...
`Group-Object` vereist niet dat de objecten die worden gegroepeerd hetzelfde Microsoft .NET Framework-type hebben. Wanneer u objecten van verschillende .NET Framework typen groepeert, `Group-Object` worden de volgende regels gebruikt:

- Dezelfde eigenschapnamen en typen.

  Als de objecten een eigenschap hebben met de opgegeven naam en de eigenschapwaarden hetzelfde .NET Framework type hebben, worden de eigenschaps waarden gegroepeerd met dezelfde regels die zouden worden gebruikt voor objecten van hetzelfde type.

- Dezelfde eigenschapnamen, verschillende typen.

  Als de objecten een eigenschap hebben met de opgegeven naam, maar de eigenschaps waarden een ander .NET Framework type hebben in verschillende objecten, `Group-Object` gebruikt het .NET Framework type van de eerste instantie van de eigenschap als het .NET Framework type voor die eigenschaps groep. Wanneer een object een eigenschap heeft met een ander type, wordt de waarde van de eigenschap geconverteerd naar het type voor die groep. Als het type conversie mislukt, wordt het object niet in de groep opgenomen.
