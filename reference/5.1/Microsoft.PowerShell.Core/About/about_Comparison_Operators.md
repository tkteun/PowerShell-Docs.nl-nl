...
Daarnaast kan in de teken reeks naar de opname groepen worden verwezen \<substitute\> .
Dit doet u door het `$` teken voor de groeps-id te gebruiken.

Twee van de manieren om naar opname groepen te verwijzen is op **nummer** en **naam**

- Op **nummer** -
   opname groepen worden van links naar rechts genummerd.
...
- Op **naam** voor het -
   vastleggen van groepen kan ook naar een naam worden verwezen.
...
> [!WARNING]
> Omdat het `$` teken wordt gebruikt in teken reeks uitbreiding, moet u letterlijke teken reeksen gebruiken met vervanging of het teken Escape `$` .
>
> ```powershell
