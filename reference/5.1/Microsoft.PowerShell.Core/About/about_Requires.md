...
Vereisen dat `Hyper-V` (versie `1.1` of hoger) is geïnstalleerd.

```powershell
#Requires -Modules @{ ModuleName="Hyper-V"; ModuleVersion="1.1" }
```

Vereist dat `Hyper-V` ( **alleen** versie `1.1` ) is geïnstalleerd.

```powershell
#Requires -Modules @{ ModuleName="Hyper-V"; RequiredVersion="1.1" }
```

Vereist dat `Hyper-V` (versie `1.1` of lager) is geïnstalleerd.

```powershell
#Requires -Modules @{ ModuleName="Hyper-V"; MaximumVersion="1.1" }
```

Vereist dat elke versie van `PSScheduledJob` en `PSWorkflow` is geïnstalleerd.

```powershell
#Requires -Modules PSWorkflow, PSScheduledJob
```

Wanneer u de `RequiredVersion` sleutel gebruikt, moet u ervoor zorgen dat uw versie teken reeks precies overeenkomt met de versie teken reeks die u wilt vereisen.
...
Het volgende voor beeld mislukt omdat **2.0.0** niet exact overeenkomt met **2.0.0.0**.
