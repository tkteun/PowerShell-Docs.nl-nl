...
Er is een bekend probleem met het gebruik van verzamelingen met **ValueFromRemainingArguments** waarbij de door gegeven verzameling wordt behandeld als één element.

In het volgende voor beeld wordt dit bekende probleem gedemonstreerd. De **resterende** para meter moet **een** bevatten bij **index 0** en **twee** bij **index 1**.
In plaats daarvan worden beide elementen gecombineerd tot één entiteit.
...
> [!NOTE]
> Dit probleem wordt opgelost in Power shell 6,2.
...
Het kenmerk **ValidateRange** geeft een numeriek bereik voor elke para meter of variabele waarde aan. Power shell genereert een fout als een wille keurige waarde buiten dat bereik valt. In het volgende voor beeld moet de waarde van de para meter **pogingen** tussen nul en tien zijn.
