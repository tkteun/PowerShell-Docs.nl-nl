...
De volgende opdracht maakt bijvoorbeeld gebruik van de teken reeks van de gebruikers agent voor Internet Explorer:
...
Zie [BasicHtmlWebResponseObject](/dotnet/api/microsoft.powershell.commands.basichtmlwebresponseobject)voor meer informatie over het object type **micro soft. Power shell. commands. BasicHtmlWebResponseObject** .

Zie [toegang tot internet via een proxy](/dotnet/framework/network-programming/accessing-the-internet-through-a-proxy)voor meer informatie over hoe .net proxy services biedt voor Power shell.
