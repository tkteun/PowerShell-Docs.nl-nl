...
> [!NOTE]
> Als u een opdracht tussen haakjes typt, wordt de automatische variabele `$?` ingesteld op `$true` , zelfs wanneer de Inge sloten opdracht zelf is ingesteld `$?` op `$false` .
> Zo wordt `(Get-Item /Nosuch); $?` onverwacht resultaat **geretourneerd**. Zie about_Automatic_Variables voor meer informatie over `$?` . [about_Automatic_Variables](about_Automatic_Variables.md)
...
Deze operator is functioneel gelijk aan `Start-Job` . In het volgende voor beeld wordt het basis gebruik van de taak operator achtergrond gedemonstreerd.
