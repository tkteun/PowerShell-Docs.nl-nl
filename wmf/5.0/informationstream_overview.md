...
Een nieuwe gestructureerde gegevensstroom kan nu worden gebruikt voor het verzenden van gestructureerde gegevens tussen een script en de host. **Write-Host** is ook bijgewerkt voor het verzenden van de uitvoer naar de stroom informatie kunt u nu vastleggen of het stilte. De nieuwe **schrijven gegevens** cmdlet (samen met een bijbehorende WriteInformation-API) die wordt gebruikt met **- InformationVariable** en **- InformationAction** algemene parameters Hiermee kunt meer flexibiliteit en mogelijkheden.
